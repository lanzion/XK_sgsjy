<template>
    <div class="achievement-detail container clearfix">
        ...
        <el-breadcrumb separator=">" class="navigation">
            ...
            <el-breadcrumb-item @click.native="redirect(pages.detail.model, { query: {id: $route.query.orgId }, path: '/achv', type: 6 })">创客建设列表</el-breadcrumb-item>
            <el-breadcrumb-item>创客建设详情</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <div class="content_r fr bgw">
            <section class="related-info">
                ...
                <ul class="list">
                    <li v-for="(item, idx) in list" @click="redirect(`#${pages.detail.model.path}/achv/detail`, { query: { id: item.id, orgId: $route.query.orgId } })" :key="idx" :title="item.name" class="related-item ellipsis">{{item.name}}</li>
    ...
    export default {
        ...
        data() {
            return {
                ...
                detail: {},
                items: [
                    { label: '建设成果', path: `${this.pages.detail.model.path}/achv` },
                    { label: '成果详情', path: '' },
                ],
                list: [],
            }
