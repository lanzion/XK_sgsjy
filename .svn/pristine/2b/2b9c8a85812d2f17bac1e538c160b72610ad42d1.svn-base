<template>
    <div class="backend-content__middle">
        ...
        <v-admin-operate :items="headBtnGroup" btnType="head"
                                         v-bind="{add: addItem, del: showMessageBox}"></v-admin-operate>
        <el-table @select="handleSelect" @select-all="handleSelect" :data="items" class="data-table back-stage-table" border
                            style="width: 100%">
...
<script>
    ...
    import setDateDialog from '@/components/admin/admin_system/set_date_dialog.vue'
    ...
    export default {
        ...
        methods: {
            ...
            async deleteItem ({data} = {}) { // 删除学年学期
                try {
                    let res = await deleteTerm({id: data.id})
                    if (res.status === 200 && res.data.code === 200) {
            ...
            async deleteItems () { // 批量删除学年学期
                try {
                    let res = await deleteTerms(this.selectedItems)
                    if (res.status === 200 && res.data.code === 200) {
                        this.$message({type: 'success', message: '成功删除学年学期', duration: 1500})
                        this.requestList()
