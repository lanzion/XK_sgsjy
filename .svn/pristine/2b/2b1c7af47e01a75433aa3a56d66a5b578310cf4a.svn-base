            ...
            <el-form ref="form" :model="form" label-width="100px" :rules="rules">
                    ...
                    <el-form-item label="兴趣组头像" style="position:relative;" required>
                        <el-col :span="14">
                            <el-upload :disabled="isPreview" class="psn-cover-uploader fl" :show-file-list="false" action :before-upload="UploadFile">
                                <img v-if="img != '' && img != null" :src="getFileUrl(img)" class="psn-cover">
                        ...
                        <el-col :span="20">
                            <el-radio-group v-model="form.interestAttr">
                                <p class="radio-box" style="margin-bottom: 10px;">
                                    <el-radio :disabled="isPreview" :label="1" :key="1">公开<span class="radio-tips">平台所有用户都可看到</span></el-radio>
                                </p>
                                <p class="radio-box">
                                    <el-radio :disabled="isPreview" :label="2" :key="2">私密<span class="radio-tips">只有兴趣组成员可以看到</span></el-radio>
                                </p>
                              </el-radio-group>
                        </el-col>
                    </el-form-item>
                ...
                <el-row>
                    <el-form-item style="margin-bottom:10px;">
                        <el-col :span="9">
                            <el-checkbox :disabled="isPreview" v-model="form.audit" style="padding-left: 3px;">加入审核</el-checkbox>
                        </el-col>
                      </el-form-item>
                </el-row>
                <!-- 兴趣组名称-->
                <el-row>
                    <el-form-item label="兴趣组名称" prop="name">
                        <el-col :span="9">
                            <el-input :disabled="isPreview" placeholder="请输入兴趣组名称，20字以内" :maxlength="20" v-model="form.name"></el-input>
                        </el-col>
                      </el-form-item>
                </el-row>
                <!-- 兴趣组简介-->
                  <el-row>
                      <el-form-item label="兴趣组简介" prop="introduction">
                          <el-col :span="23">
                            <el-input :disabled="isPreview" type="textarea" :maxlength="500" v-model="form.introduction" placeholder="请输兴趣组简介，500字以内" style="height: 100px;"></el-input>
                        </el-col>
                      </el-form-item>
                  </el-row>
                  <el-row>
                      <el-form-item label="分类" prop="interestType">
                          <el-col :span="9">
                              <multi-select :disabled="isPreview" style="width:100%;display:inline-block;" :selectData.sync="form.interestType" :ifAll='false' :model='"interest"'></multi-select>
                          </el-col>
                      </el-form-item>
                  </el-row>
                  <el-row>
                      <el-form-item label="相关技术" prop="technologys">
                          <el-col :span="9">
                              <el-select :disabled="isPreview" multiple placeholder="请选择相关技术" v-model="form.technologys" style="width:100%;">
                                  <el-option
                  ...
                  <el-form-item v-if="!isPreview">
                      <button class="psn-confirm-btn" type="button" @click.prevent="createInterest(0)">保存</button>
                      <button class="psn-confirm-btn" type="button" @click.prevent="createInterest(1)" v-if="!isManage">提交并审核</button>
                      <button class="psn-cancel-btn" @click.prevent="$router.go(-1)" v-if="!isManage">取消</button>
                </el-form-item>
            </el-form>
        </div>
    </section>
...
<script>
    ...
    import { addInterestGroup, requestInterestDetail, modifyInterestGroup } from '@/service/interest.js'
            ...
            *     "cover": "1.兴趣组图片",
            ...
            createInterest(isSubmit) {
                if (localStorage.getItem('loginInfo')) {
                    const sucMsg = isSubmit === 0 ? '保存成功' : '提交成功'
                    this.$refs.form.validate((valid) => {
                        if (valid) {
                            for (const i in this.form.technologys) {
