...
<script>
  import {queryLabelDetail, addTag, editTag} from '@/service/admin_system.js'
  ...
  export default {
    ...
    methods: {
      async queryDetail () { // 查询标签详情
        let res = await queryLabelDetail({id: this.id})
        if (res.status === 200 && res.data.code === 200) {
          const {name = '', labelType = '1'} = res.data.entity || {}
          this.form.name = name
      ...
      submit () { // 保存新增的标签
        this.$refs.form.validate((valid) => {
          if (valid) {
            const handler = this.id && editTag || addTag
            const label = this.id && '修改' || '添加'
            ...
            handler({...this.form, id: this.id}).then((res) => {
              if (res.status === 200 && res.data.code === 200) {
                this.$message({
                  ...
                  message: `成功${label}标签`,
                  duration: 1000,
                  onClose: () => this.$router.push('/system/tag')
                })
                return
              }
