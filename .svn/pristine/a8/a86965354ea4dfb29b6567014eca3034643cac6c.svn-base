<template>
    <div class="studioArticle container clearfix">
        ...
        <!-- 发布修改 -->
        <el-dialog title="发布研究" :visible.sync="pubVisible" size="small" :before-close="closePopup">
                    ...
                    uploader.off('uploadSuccess').on('uploadSuccess', function (file, res) {
                        if (res.code === 200) {
                            if (res.appendInfo.documentId) {
                                _this.form.studioAttachments.push({ type: 2, name: file.name, resourceId: res.appendInfo.resourceId, documentId: res.data.appendInfo.documentId })
                            } else {
