            ...
            <div v-show="screeningDropdown" class="bs-screening_wrap">
                <el-form :inline="true" :model="dataParams" class="bs-screening_box">
                    ...
                    <el-form-item label="级别">
                        <el-select v-model="dataParams.grade" placeholder="请选择级别" filterable clearable>
            ...
            <el-table-column prop="name" label="课程名称" show-overflow-tooltip>
                <template scope="scope">
                    ...
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/course/detail', query: {id: scope.row.id} }">{{scope.row.name}}</router-link>
                    </span>
                </template>
            </el-table-column>
            <el-table-column label="课程分类" show-overflow-tooltip>
                <template scope="scope">
                    {{scope.row.courseTypeName}}
                </template>
            </el-table-column>
            <el-table-column prop="technologyName" label="技术" width="100" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestCourseList, requestCourseAuditDetail, hotCourse, requestCourseCancelAudit, delCourse } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                requestCourseList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            /*
             * 推荐
             * { "relationIds": ["课程ID"] , "recommendType": 7 }
             */
            doRecommend({data: item} = {data}) {
                let params = { relationIds: [], recommendType: 7 },
                    flag   = false;
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过且未推荐的课程' })
                    } else {
                        this.recommendOperate(item, params, 7)
                    }
                } else {
                    this.$message({ message: '请选择要推荐的课程' })
                }
            },
            //取消推荐
            cancelRecommend({data: item} = {data}) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '您选择的课程中包含了未推荐的课程' })
                    } else {
                        this.recommendOperate(item, params, 7, true)
                    }
                } else {
                    this.$message({ message: '请选择要取消推荐的课程' })
                }
            },
            /*
             * 审核记录
            ...
            doRecord({data: item} = {data}) {
                requestCourseAuditDetail({courseId: item.id}).then(response => {
                    if (response.data.code == 200) {
            ...
            handleHot({data: item, model} = {data, model}) {
                ...
                if (item.length) {
                    ...
                    if (flag) {
                        this.$message({ message: '只能设置审核通过的课程' })
                    } else {
                        this.$confirm(`此操作将该课程${handleTxt}热门，是否继续？`, `${handleTxt}热门`, {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning',
                            customClass: '__custom-msg-box'
                            ...
                            hotCourse(params).then(response => {
                                if (response.data.code == 200) {
                                    item.forEach(x => {
                                        x.isHot = 1
                                    })
                    ...
                    this.$message({ message: '请选择要设置的课程' })
                }
            },
            // 撤销审核
            cancelAudit(item) {
                this.$confirm('此操作将撤销该课程的审核，是否继续？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning',
                    customClass: '__custom-msg-box'
                    ...
                    requestCourseCancelAudit(params).then((res) => {
                        if (res.data.code === 200) {
