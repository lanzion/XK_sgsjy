...
<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" @keyup.native.enter="changePage(1)" v-model="dataParams.title" placeholder="请输入标题"></el-input>
            ...
            <div v-show="screeningDropdown" class="bs-screening_wrap">
                <el-form :inline="true" :model="dataParams" class="bs-screening_box">
                    <el-form-item label="级别">
                        <el-select v-model="dataParams.subjectLevel" placeholder="选择级别" clearable>
                    ...
                    <el-form-item label="学校" v-show="!isSchool">
                        <el-select v-model="dataParams.schoolId" placeholder="请选择学校" clearable>
       ...
       <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column label="标题" show-overflow-tooltip>
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/subject/guide/detail', query: {id: scope.row.id} }">{{scope.row.title}}</router-link>
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="省市区" show-overflow-tooltip>
                <template scope="scope">
                    {{scope.row.provinceId}}{{scope.row.cityId}}{{scope.row.areaId}}
                </template>
            </el-table-column>
            <el-table-column prop="jobName" label="发布单位"></el-table-column>
            <el-table-column label="申报状态" width="100">
                <template scope="scope">
                    <el-tag :class="['__color-'+auditStatusStyle[scope.row.status]]">
                        {{scope.row.status | translate(subjectGuideStageDL)}}
                    </el-tag>
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestSubjectList, requestProjectAudit, delSubject } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestSubjectList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            del ({data: item} = {data}) {
                ...
                if (item.length) {
                    ...
                    this.$message({message: '请选择要删除的申报指南'})
                }
            },
            // 重置搜索表单
            resetSearchForm() {
        ...
        computed: {
            ...
            ...mapState('dictionaryCommon', {
                ...
                subjectGuideStageDL (state) {
                    let dicList = (state['subjectGuideStageDicList'] || {}).dicList || []
                    return dicList
                }
            })
        }
