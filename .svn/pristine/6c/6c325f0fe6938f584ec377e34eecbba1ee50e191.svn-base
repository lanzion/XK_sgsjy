            ...
            <div class="table-box mes_ul">
                <el-table class="data-table" :data="dataList" style="width: 100%">
                    ...
                    <el-table-column prop="date" :label="table_title[3]" show-overflow-tooltip>
                        <template scope="scope">{{ scope.row.date | dateFormat('yyyy-MM-dd')}}</template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="80" v-if="flag === '/resource'">
                        <template scope="scope"><a :href="downloadUrl(scope.row.resourceId, scope.row.prop0)" :download="scope.row.prop0">下载</a></template>
                    </el-table-column>
                </el-table>
            </div>
