<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="typeDatas" title="课程分类" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="gradeDatas" title="课程级别" centerTitle="课程(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="userName" label="教师" show-overflow-tooltip></el-table-column>
                <el-table-column prop="gradeName" label="级别" show-overflow-tooltip></el-table-column>
                <el-table-column prop="num" label="学员数" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestCourseType, requestCourseGrade, requestCourseDetail } from '@/service/statis_school.js'

    export default {
        name: 'statisticsCourseSchool', // 课程统计
        components: {
        ...
        methods: {
            ...
            // 获取课程级别统计数据
            getStatusData() {
                requestCourseGrade(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data  = response.data.entity || {},
                            _datas = _data.data || [],
                            _yAxis = []
