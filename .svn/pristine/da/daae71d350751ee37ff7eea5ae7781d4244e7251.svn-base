  ...
  export default {
    ...
    methods: {
      async requestExamQuestionList () { // 获取考题列表
        let res = await requestExamQuestionList({sectionId: this.sectionId})
      ...
      async submitTest () { // 提交试卷
        ...
        this.answers.forEach(item => item._myOptions && (item.myOptions = item._myOptions.join(',')))
        try {
          ...
          let res = await submitTesting({sectionId: this.sectionId, examinationPaperList: this.answers})
          if (res.data.code === 200) {
...
//          this.$message.error('提交试卷失败，请重试')
      ...
      generateAnswerArr () { // 生成与考题对应的答案数据
        this.answers = this.questions.map((item, i) => {
          let _flag = (item.questBank || {}).bankType === 2
          let _key = _flag && '_myOptions' || 'myOptions'
          return {
            id: item.id,
