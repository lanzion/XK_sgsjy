<template>
    <div class="backend-content__middle">
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search.name" @keyup.native.enter="changePage(1)" placeholder="请输入学生姓名"></el-input>
            ...
            <el-table-column prop="account" label="账号" sortable show-overflow-tooltip>
                <template scope="scope">
                    ...
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/base/student/detail', query: {id: scope.row.id} }">{{scope.row.account}}</router-link>
                    </span>
                </template>
            </el-table-column>
        ...
        methods: {
            ...
            doRecommend({data: item} = {data}) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过、未锁定和未推荐的学生', duration: 2000 })
                    } else {
            ...
            cancelRecommend({ data: item } = { data }) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '您选择的学生中包含了未推荐的学生', duration: 2000 })
                    } else {
            ...
            lockOperate({data: item, model} = {data, model}){
                ...
                item = JSON.stringify(item) === '{}' ? this.checkedItems : [item]
                if (item.length) {
                    this.$confirm(`此操作将${text}该学生，是否继续？`, text, {
                    ...
                    this.$message({ message: `请选择要${text}的学生` })
                }
            },
            /*
             * 审核
