    ...
    <section id="interestsArticle" class="bgc_w">
        ...
        <section class="func">
            ...
            <div class="func_r clearfix">
                ...
                <!-- 发表 -->
                <router-link class="func_r_pub icon-btn-i-feedback" v-if='isMember' :to="{path:'article/publish', query: {id: $route.query.id}}"> 写文章 </router-link>
            </div>
        </section>
        <!-- 文章列表 -->
        <ul class="article_list" v-if="articleList.length">
            <li v-for="(item, index) in articleList" :key="item.id" class="article_li">
                <router-link class="article_li_title" :to="{path: detailPath, query: {id: item.id, orgId: $route.query.id}}" target="_blank">
                ...
                <div class="article_li_func clearfix">
                    ...
                    <span class="fr article_li_func_del clearfix" v-if="item.createId == $ls.get('loginUId')">
                        <v-del :delData="{ url: '/maker/interests/article/del', name: '文章', params: { id: item.id } }" class="article_del fr" @callback="getListData" :isIcon="false"></v-del>
                        <router-link tag="span" :to="{ path: '/space/interest/article/edit', query: { id: $route.query.id, articleId: item.id } }" class="article_edit icon-btn-edit2 fr">编辑</router-link>
                    </span>
                </div>
            </li>
        </ul>
        ...
        <el-dialog title="管理分类" :visible.sync="classifyPopup" size="tiny" class="classify" :before-close="clearModifyIndex">
            ...
            <ul class="classify_list">
                <li v-for="(item, index) in classify.options" class="classify_li">
                    ...
                    <v-del :delData="{ url: '/maker/interests/article/type/del', name: '分类', params: { id: item.id } }" class="article_del fr" @callback="getClassifyData" v-if="modifyIndex != index"></v-del>
                    <span class="icon-btn-i-edit2 fr" @click="modifyParam( item.id, index, item.name )" v-if="modifyIndex != index"></span>
                    <span class="confirm-btn fr" @click="addClassify(item, index)" v-if="modifyIndex == index">确定</span>
                </li>
            </ul>
...
export default {
    ...
    data() {
        return {
            ...
            search: null,
            flagUnshift: true,
            detailPath: `${this.pages.detail.interest.path}/article/detail`
        }
    },
    components: {
        'v-search': search,
...
#interestsArticle {
    ...
    .article_list {
        ...
        .article_li {
            ...
            .article_li_func {
                ...
                .icon-i-time, .icon-input-user, .article_edit, .article_del {
                    line-height: 30px;
                    height: 100%;
                    padding: 0 10px;
                }
    ...
    .classify {
        ...
        .classify_add {
            ...
            width: 100%;
            height: 40px;
            border: 1px dotted nth($default-color, 1);
