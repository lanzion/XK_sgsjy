...
<script>
    ...
    import { requestResourceList, delResource, submitResource } from '@/service/resource.js'
                ...
                requestResourceList(data, this.pageParam).then((res) => {
                    if (res.data.code === 200) {
                        ...
                        this.datas.forEach((item) => {
                            // 技术转译
                            dataTranslation(item.technology, 'technicalClassification', { key: 'id' }).then((response) => {
                                item.technologyName = response
                            })
                        })
                    }
                })
