...
<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="dataParams.demandName" @keyup.native.enter="changePage(1)" placeholder="请输入需求名称"></el-input>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="demandName" label="需求标题" show-overflow-tooltip width="200">
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/demand/detail', query: {id: scope.row.id} }">{{scope.row.demandName}}</router-link>
                    </span>
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestDemandAuditList, requestDemandAudit, requestDemandCancelAudit } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestDemandAuditList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            resetSearchForm() {
                ...
                this.dataParams = Object.assign({}, this._dataParams, { name: this.dataParams.demandName })
