<template>
    <div class="wrapper pr dpd15">
        <div v-if="!isEditing" class="outline-wrapper">
            <ul v-if="courseData.length !== 0" class="chapters">
                <li v-for="(chapter, index) in courseData" :key="index" class="chapter-item">
                    <h3 class="chapter">
                        {{chapter.name}}
                        <span v-if="Array.isArray(chapter.courseHourSectionList) && chapter.courseHourSectionList.length && isAdmin" class="section-num fr">共{{chapter.courseHourSectionList.length}}个课时</span>
                    </h3>
                    <ul class="sections">
                        <li v-if="Array.isArray(chapter.courseHourSectionList)" v-for="(section, i) in chapter.courseHourSectionList" :key="i" class="section-item">
                            <i v-if="section.status !== 0 && !isAdmin" class="icon-has-learned icon-form-correct"></i>
                            <a class="section fl" href="javascript:;">
                            <span v-if="isAdmin">【{{handleSectionType(section.sectionType)}}】</span>{{section.name}}</a>
                            <template v-if="!isAdmin && loginUId != courseInfo.createId">
                                <div v-if="!$route.path.startsWith('/space/course')" class="operate fr">
                                    <div v-if="section.status === 0" class="operate-has-not-learned">
                                        <a v-if="section.sectionType === '2'" @click="goToStudy({sectionId: section.id, type: section.sectionType})" class="link" href="javascript:;" title="练习"><i class="icon-btn-link"></i></a>
                                        <a v-else-if="section.sectionType === '1'" @click="goToStudy({sectionId: section.id, type: section.sectionType})" class="video" href="javascript:;" title="视频"><i class="icon-btn-watch"></i></a>
                                    ...
                                    <div v-else class="operate-has-learned">
                                        <p v-if="section.sectionType === '2'" class="can-test-again">
                                            <a @click="goToStudy({sectionId: section.id, type: section.sectionType, lastResult: true})" href="javascript:;" class="btn-last-test-result btn">上次结果</a>
                                            <a @click="goToStudy({sectionId: section.id, type: section.sectionType})" href="javascript:;" class="btn-test-again" title="再次练习"><i class="icon-btn-link"></i></a>
                                        </p>
                                        <a v-else-if="section.sectionType === '1'" @click="goToStudy({sectionId: section.id, type: section.sectionType})" href="javascript:;" class="btn-learn-again" title="再次学习"><i class="icon-btn-watch"></i></a>
                                    </div>
                                </div>
                            </template>
                        </li>
        ...
        <course-release-add v-else @on-cancel-editing="isEditing = false" @on-edit-success="handleEditSuccess" v-bind="{editCourseData, firstStepData}"/>
...
<script>
    ...
    import courseReleaseAdd from 'components/course/course_release_add.vue'
    ...
    export default {
        ...
        methods: {
            ...
            async requestCourseDetail() { // 修改目录，获取对应id课程的数据
                const id = this.id
                if (id) {
                    const res = await requestBackendCourseInfo({ id })
            ...
            goToStudy({ type, sectionId, lastResult } = {}) { // 去学习
                ...
                if (this.checkRegisteredOrNot()) {
                    let _path = this.pages.detail.course.path + (type === '1' ? '/video/outline' : '/test')
                    lastResult && (_path += '/a')
            ...
            initData() { // 修改目录，课程基本信息
                const _data = this.editCourseData
                if (_data.id) {
                    const obj = {}
                    obj.name = _data.name || ''
