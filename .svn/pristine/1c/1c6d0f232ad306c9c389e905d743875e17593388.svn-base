        ...
        <el-form ref="form" :model="form" label-width="80px" :rules="rules">
            ...
            <el-row>
                <el-form-item label="标题" prop='name'>
                    <el-col :span="9">
                        <el-input placeholder="请输入需求标题，不超过30字" v-model.trim="form.name" :maxlength="30"></el-input>
                    </el-col>
                </el-form-item>
            </el-row>
            <!-- 所属分类 -->
            ...
            <el-row>
                <el-form-item label="需求封面" prop="cover">
                    <el-col :span="14">
                        <el-upload class="cover-uploader fl" :show-file-list="false" action :before-upload="UploaCover">
            ...
            <el-row>
                <el-form-item label="附件" prop="resourceList">
                    <el-col :span="15">
                        ...
                        <el-upload class="upload-demo" action :file-list="form.demandResource" :on-remove="resourceListRemove" :before-upload="UploadFile">
            ...
            <!-- 按钮部分 -->
            <el-form-item>
                <button class="publish-btn" @click="onSubmit(1)" type="button">提交审核</button>
                <button class="publish-btn" @click="onSubmit(0)" type="button">保存</button>
                <button class="btn" @click="$router.push({ path: '/admin/demand/pending' })">取消</button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
import dataTranslation from '@/js/dataTranslation.js'
    export default {
        ...
        data () {
            var changeContent = (rule, value, callback) => {
                    ...
                    uploader.off('uploadSuccess').on('uploadSuccess', function (file, res) {
                        if (res.code === 200) {
                            if (res.appendInfo.documentId) {
                                _this.form.demandResource.push({name: file.name, resourceId: res.appendInfo.resourceId, documentId: res.appendInfo.documentId, status:'finished'})
                            } else {
                                _this.form.demandResource.push({name: file.name, resourceId: res.appendInfo.resourceId, status:'finished'})
                            }
                        }
                    })
                    uploader.off('uploadError').on('uploadError', function (file, reason) {
