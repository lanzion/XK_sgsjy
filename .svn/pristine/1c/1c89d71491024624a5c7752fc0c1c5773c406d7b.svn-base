<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="typeDatas" title="资源分类" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="formatDatas" title="资源格式" centerTitle="资源(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="num" label="资源数" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestResourceType, requestResourceFormat, requestResourceDetail } from '@/service/statis_school.js'

    export default {
        name: 'statisticsResourceSchool', // 资源统计
        components: {
        ...
        methods: {
            ...
            getTypeData() {
                requestResourceType(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        ...
                        let _result = {
                            subtext: `资源分类: ${_data.dataAxis ? _data.dataAxis.length : 0}，资源总数: ${_data.total || 0}个`,
            ...
            // 获取资源格式统计数据
            getStatusData() {
                requestResourceFormat(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data  = response.data.appendInfo.data || {},
                            _datas = _data.data || [],
                            _yAxis = []
