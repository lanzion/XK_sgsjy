<template>
    <div class="container listBox">
                ...
                <div class="maker_li bgw">
                    <div class="maker_li_personal clearfix" @click="goToPersonSpace(i)">
                        ...
                        <div class="maker_li_personal_msg fl">
                            <p class="maker_li_personal_msg_name" v-html="filterKeyWords(i.teacherName, keyWords)"></p>
        ...
        <div class="no-data" v-else>暂无教师空间数据</div>
...
import { requestTeacher } from '@/service/space.js'
...
export default {
    ...
    methods: {
        ...
        // 请求列表数据
        getData() {
            requestTeacher({ teacherName: this.keyWords }, this.pageParam).then((response) => {
        ...
        goToPersonSpace(i) {
            ...
            if (loginUid === i.userId) {
                ...
                this.redirect(this.pages.detail.teacher, { query: { id: i.orgId, uid: i.userId } })
            }
        }
    },
    watch: {
    ...
    beforeRouteLeave(to, from, next) {
        if (to.path === '/space/teacher/dynamic' && (to.query.uid !== undefined && to.query.uid !== this.loginUserId)) {
