      ...
      <ul>
        <list-row v-for="(item, index) in lists" :item="item" :key="index">
          <h3 slot="title" class="name ellipsis">
            <router-link :to="{path: '/space/resource/details', query: {id: item.id}}" :title="item.name" class="link">
              {{item.name || '暂无'}}
            </router-link>
          </h3>
          ...
          <div slot="operate" class="operate pa">
            ...
            <router-link :to="{path: '/space/resource/details', query: {id: item.id}}" class="btn">预览</router-link>
            <a v-if="$ls.get('loginInfo')" :href="downloadUrl(item.fileId, item.name)" :download="item.name || +new Date" class="btn" @click="downloadAdd(item.id,index)">下载</a>
          </div>
        </list-row>
      </ul>
  ...
  export default {
    ...
    methods: {
      async requestList () { // 获取列表数据
        let res = await requestcFrontendResourceList({
          ...this.selected,
