    ...
    <section class="evaluation">
        <article class="rate-header">
            ...
            <div class="rate-header-content table-layout">
                ...
                <ul class="rate-tag table-cell">
                    <li class="rate-tag-item" v-for="(tags, index) in evaluationDatas.optionCount" v-bind:class="{'active': curTagId === tags.labelId}" @click="changeTag(tags.labelId)" :key="index">{{tags.labelName}}<span>（{{tags.evalNum}}）</span></li>
                </ul>
            </div>
        </article>
        <article class="evaluation-form">
            <form>
                ...
                <el-input type="textarea" v-model="addEvaluateForm.content" placeholder="评论内容不超过100字" :rows="3"></el-input>
                <div class="btn-group">
                    <button type="reset" class="psn-cancel-btn" @click="resetEvaluate">取消</button>
                    <button type="button" class="psn-confirm-btn" @click="addEvaluate">发表</button>
                </div>
            </form>
        </article>
            ...
            <ul class="evaluation-list" v-else>
                <li class="evaluation-item clearfix" v-for="(list, lIndex) in evaluationList" :key="lIndex">
                    ...
                    <div class="btn-group" v-if="list.createId != userId">
                        <v-report :report-data="{'url': 'maker/projectEvaluate/addEvaluate', params: {'commentId': list.id }}"></v-report>
                    </div>
...
export default {
    ...
    watch: {
        ...
        evaluationDatas: {
            handler: function () {
                if (this.evaluationDatas.scoreCount !== undefined) {
                    const _scoreCount = this.evaluationDatas.scoreCount
    ...
    computed: {
        ...
        curTotalEvalu: function () {
            const _item = this.choiceStars.concat().filter(item => item.code === this.curStar)[0]
            if (_item !== undefined) {
                return _item.ct
            }
        }
    ...
    methods: {
         ...
         */
        getEvaluateCount() {
            const url = this.evaluateData.dataUrl.evaluateCount
            const postData = { [this.evaluateData.params]: this.evaluateData.id }
            this.$http.post(url, postData).then((response) => {
                const appendInfo = response.data.appendInfo || {}
         ...
         */
        getEvaluateList() {
            const url = this.evaluateData.dataUrl.evaluateList + '?pageSize=10&pageNum=' + this.evaluateListPageNum
            const postData = { [this.evaluateData.params]: this.evaluateData.id, score: this.curStar, labelId: this.curTagId }
            this.$http.post(url, postData).then((response) => {
        ...
        addEvaluate() {
            if (this.$ls.get('loginInfo')) {
                ...
                const flag = this.vaildAddForm(params)
                if (flag !== true) {
            ...
            } else if (this.addEvaluateForm.content.trim() !== '') {
                this.$confirm('您尚未登录，是否跳转至登录页？', '提示', {
...
$color-data: (
    ...
    lightBlue: #23b8ff #dbf1ff #2797ed
);
