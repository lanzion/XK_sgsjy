<template>
    <article id="projectIntro" class="projectIntroPanel clearfix">
        <div :class="[ifAdmin ? 'projectIntroAdmin':'projectIntroPrev']" :style="{backgroundImage:'url('+ getFileUrl((projectDetail.imgUrl?projectDetail.imgUrl:null))+')'}" title="点击查看原图" @click="visible.show = true"></div>
        ...
        <img-preview :visible.sync="visible" :imgUrl="projectDetail.imgUrl"></img-preview>
    </article>
</template>
...
export default {
    ...
    methods: {
        addMemberApply() {
            const flag = Number(this.status.sysTime) >= Number(this.projectDetail.enrollStartDate) && Number(this.status.sysTime) <= Number(this.projectDetail.enrollEndDate)
            if (!flag) {
    ...
    mounted() {
        ...
        this.ifLogin = this.$ls.get('loginInfo') !== null
    },
    filters: {
        stutas(status) {
            let txt = null
