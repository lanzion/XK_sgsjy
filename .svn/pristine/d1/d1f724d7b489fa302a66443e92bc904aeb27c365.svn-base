      ...
      <ul>
        <list-row v-for="(item, index) in lists" :item="item" :item-height="156" :key="index">
          <div slot="content" class="content table-cell">
            <p>活动名称：
              <router-link :to="{path: '/space/activity/details/member', query: {id: item.id}}" :title="item.name"
            ...
            <p>自评语：{{item.remark}}</p>
            <p>实践成果：
              <router-link v-for="(result, i) in item.resultList" :key="i" :title="result.name"
                           :to="{path: `${isAdmin && '/admin' || '/space'}/works/details`, query: {id: result.worksId}}"
