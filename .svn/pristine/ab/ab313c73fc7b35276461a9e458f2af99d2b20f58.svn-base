<template>
    <div class="submit-form backend-content__middle">
        <el-form ref="form" :model="form" label-width="160px" :rules="rules">
            ...
            <!-- 描述 -->
            <el-row>
                <el-form-item label="项目类别" prop="description">
                    <el-col :span="12">
            ...
            <!-- 预留字段 -->
            <el-row>
                <el-form-item label="上传页面的预留字段" prop="fieldList">
                    <el-col :span="12">
                        <el-row class="evaluate-title">
                            <el-col :span="8">字段名称</el-col>
                            <el-col :span="12">字段说明</el-col>
                            <el-col :span="4">操作</el-col>
                        </el-row>
                        <el-row class="evaluate-list" v-for="(item, index) in form.fieldList" :key="index">
                            <el-col class="evaluate-input-box" :span="8">
                                <el-input v-model.trim="item.name"></el-input>
                            </el-col>
                            <el-col class="evaluate-input-box" :span="12">
                                <el-input v-model.trim="item.description"></el-input>
                            ...
                            <el-col :span="4">
                                <i class="handler icon-btn-delete" @click="deleteField(index)"></i>
                            </el-col>
                        </el-row>
                        <button type="button" class="evaluate-add i-form-button other-c" @click="appendField"><i class="icon-btn-add"></i> 添加</button>
                    </el-col>
                    <el-col :span="12">
                        <p class="hint">
                            <i class="table-cell icon-form-tip"></i>
                            <span class="table-cell">
                                <p>可增加预留字段，让参赛者在上传作品时填写</p>
                            </span>
                        </p>
                    </el-col>
                </el-form-item>
...
export default {
    name: 'contestEventAddOnline',
    data() {
        const validateEvaluationList = (rule, value, callback) => {
        ...
        const validateField = (rule, value, callback) => {
            value = value.filter(x => (x.name.length || x.description.length))
            if (value.length) {
                let completeItems = value.filter(x => (x.name.length && x.description.length))
                if (value.length === completeItems.length) {
                    callback()
                } else {
                    callback(new Error(`字段名称和字段说明不能为空`))
                }
            } else {
                callback(new Error(`请至少输入 1 个完整的预留字段`))
            }
        }
        return {
            // 表单数据
            ...
            rules: {
                ...
                fieldList: [
                    { type: 'array', required: true, message: '请至少输入 1 个完整的预留字段', trigger: 'blur' },
    ...
    methods: {
        ...
        getDetail() {
            requestContestItemDetail({id: this.form.id}).then(res => {
                if (res.data.code == 200) {
                    ...
                    _form.fieldList = details.fieldList.map(x => {
                        let item = {}
                        item.name = x.name
                        item.description = x.description
                        return item
        ...
        onSubmit(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    const _evaluationList = this.form.evaluationList.filter(x => (x.name.length && x.description.length))
                    const _fieldList = this.form.fieldList.filter(x => (x.name.length && x.description.length))
                    this.$set(this.form, 'evaluationList', _evaluationList)
                    this.$set(this.form, 'fieldList', _fieldList)
