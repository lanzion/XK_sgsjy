    ...
    <section id="myAdminFocus">
        ...
        <el-dialog title="设置分组" :visible.sync="setClassifyPopup" size="tiny" class="setClassify" :lock-scroll="true">
            ...
            <div class="button_group">
                <button type="button" class="psn-cancel-btn" @click="setClassify">取消</button>
                <button type="button" class="psn-confirm-btn" @click="saveClassify">保存</button>
            </div>
        </el-dialog>
...
<script>
import { groupList, friendGroup, deleteFriend, friendList } from '@/service/my.js'
        ...
        setClassify(item) {
            if (this.checkedList.length !== 0 || item) {
                ...
                if (this.setClassifyPopup) {
                    ...
                    const arrName = this.checkedList.map(i => i.name)
