	<template>
	<el-dialog :visible.sync="visible.show" id="publish_resource" title="发布资源" class="dialog" @close="close">
                                    ...
                                    <section class="table-box">
                                        <el-table
                                            ...
                                            <el-table-column label="资源名称" show-overflow-tooltip>
                                                <template scope="scope"><span class="res-name">{{scope.row.name}}</span></template>
                                            </el-table-column>
...
<script>
	...
	import { requestTypeList,publishResource,requestResShareDetail,modifyStudioResource } from '@/service/studio.js';
	export default{
		...
		mounted(){
			let fileElement = this.$el.querySelectorAll('[type="file"]')
