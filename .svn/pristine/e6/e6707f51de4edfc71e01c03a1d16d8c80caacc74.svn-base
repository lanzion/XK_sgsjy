  ...
  export default {
    ...
    methods: {
      async requestWorksList () { // 获取列表数据
        let res = await requestBackendWorksList({auditStatus: 1}, {pageSize: 999, pageNum: 1})
      ...
      async deleteItem (id) { // 删除已经关联的成果
        let handler = this.module === 'practice' && deletePracticeResult || deleteStudyResult
        let res = await handler({id})
      ...
      submit () { // 提交
        let handler = this.module === 'practice' && addPracticeResult || addStudyResult

        // 关联的是作品id
        let selected = this.$refs.worksTree.getCheckedNodes().map(item => item.id) || []
        ...
        let practiceWorksArr = selected.map(item => ({busId: this.id, worksId: item})) || []
        let options = this.module === 'practice'
          ? practiceWorksArr
          : {studyId: this.id, works: selected}
