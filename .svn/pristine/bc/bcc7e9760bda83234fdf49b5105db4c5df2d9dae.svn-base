<template>
    <el-dialog class="gouda-list" title="勾搭列表" :visible.sync="hookUpWithPopup.isVisible" size="small" lock-scroll>
...
export default {
    ...
    methods: {
        ...
        getListData() {
            hookupWithList({ linkMan: this.search, status: this.selected, busId: this.curHookId, type: this.type }, this.pageParam).then((res) => {
    ...
    computed: {
        ...
        ...mapState('dictionaryCommon', {
            hookUpWithStatusOptions(state) {
                const dicList = (state.hookUpWithStatusDicList || {}).dicList || []
                return dicList
            }
        }),
        params: function () {
