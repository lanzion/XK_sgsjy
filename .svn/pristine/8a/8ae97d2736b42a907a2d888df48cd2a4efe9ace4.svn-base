<template>
  <!--绑定key，切换课程时，避免组件缓存-->
  <div :key="$route.query.id" class="courseDetails">
    <v-breadcrumb v-if="!isAdmin"></v-breadcrumb>
    <v-course-intro @on-register-success="handleRegisterSuccess" @on-be-creator="handleRegisterSuccess"
                    :has-registered="hasRegistered"></v-course-intro>
    <div v-if="!isAdmin && hasRegistered && loginUId != courseInfo.createId" class="goto-study clearfix">
      <div class="progress-wrapper fl">
        <el-progress class="progress-bar" :text-inside="true" :stroke-width="22"
                     :percentage="progressInfo.proccess"></el-progress>
        <p class="tip">
    ...
    <div class="clearfix">
      ...
      <div v-if="!isAdmin" class="content_r">
        <v-aside></v-aside>
      </div>
    </div>
  </div>
...
<script>
  import { checkHasAppliedOrNot, queryStudyingProgress } from '@/service/course.js'
  import { mapState, mapActions, mapGetters } from 'vuex'
  import breadcrumb from 'components/common/breadcrumb.vue'
  import tabs from 'components/common/classificationList/classification/tabs.vue'
  import courseIntro from 'components/common/module/course/intro.vue' // 课程介绍
    ...
    methods: {
      ...
      async checkHasAppliedOrNot () { // 检查是否已经报名
        ...
        if (res.data.code === 200) {
          let _appendInfo = res.data.appendInfo || {}
      ...
      startStudying () { // 去学习
        let sectionId = this.courseOutline[0] && this.courseOutline[0].courseHourSectionList[0].id || ''
        let type = this.courseOutline[0] && this.courseOutline[0].courseHourSectionList[0].sectionType || ''
        if (!sectionId) {
        ...
        if (type == '1') {
          this.$router.push({path: '/video', query: {id: this.id, sectionId}})
        } else {
          this.$router.push({name: 'courseTest', query: {id: this.id, sectionId}})
        }
      }
    },
  }
