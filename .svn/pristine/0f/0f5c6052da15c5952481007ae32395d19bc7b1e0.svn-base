<template>
    <div id="resourceDetail" class="theme-orange clearfix" v-bind:class="{'backend-content__middle': !isFront,'container': isFront }">
        <div class="content-left fl" v-bind:class="{'width100':!isFront}">
            <!-- 预览 -->
            <div class="preview-box bgw">
                <doc-reader v-if="detailsData.documentId" v-bind="{docId: detailsData.documentId, width: 876}"></doc-reader>
                <div v-else class="preview-img" :style="{backgroundImage: `url(${getFileUrl(detailsData.resourceId) || listDefault})`}"></div>
            </div>
            <!-- 资源信息 -->
...
export default {
    ...
    data() {
  	    return {
            ...
            morePath:'/studioMemberDetail/resource'
  	    }
    },
    created() {
