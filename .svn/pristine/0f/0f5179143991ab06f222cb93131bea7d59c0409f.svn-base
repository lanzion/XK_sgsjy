...
// import { requestAchievementCommentList, addAchievementComment, requestFeatureCommentList, addFeatureComment } from '@/service/model_school.js'
    ...
    methods: {
        ...
        showPublish(arr) {
            if (this.$ls.get('loginInfo')) {
                if (this.show[0] !== null && this.show[1] === null && arr[0] === this.show[0] && arr[1] === null || this.show[0] !== null && this.show[1] !== null && arr[0] === this.show[0] && arr[1] === this.show[1]) {
                    this.show = [null, null]
                } else {
                    this.show = arr
                }
            } else {
                window.location.href = `${this.pages.login.url}?callback=${encodeURIComponent(window.location.href)}`
        ...
        // 获取评论列表 workId:作品Id status:状态 activityId:活动Id
        ...
        async getAllReplyList(index, pId) {
            ...
            const res = await this.$http.post(url, { [this.commentProps.parmasKey]: this.id, pId, status: 1 })
            if (res.data.code === 200) {
        ...
        // 点赞/取消点赞 commentId：评论id status：状态(1:启用0:禁用)
