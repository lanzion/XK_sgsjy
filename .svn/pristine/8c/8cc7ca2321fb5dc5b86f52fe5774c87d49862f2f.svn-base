    ...
    <aside class="aside">
        <div class="course-rel">
            <h3 class="title">
                ...
                <a @click="redirect({ url: `/#/education/course` }, { type: 3 })" class="btn-more fr" href="javascript:;">更多>></a>
            </h3>
            <ul class="course-rel-wrapper">
                <li v-for="(item, index) in lists" @click="handleSwitch(item.id)" :key="index" class="item">
                    ...
                    <p class="about clearfix">
                        <span class="free fl">{{item.isFree === '1' && '免费' || '收费'}}</span>
                        <span class="num fr"><i class="icon-i-creator"></i>{{item.num || 0}}人学习 </span>
                    </p>
                </li>
        ...
        <!--<div class="practice-rel">
            ...
            <ul class="practice-rel-wrapper">
                <li class="item">
                    <figure class="pic">
                        <img src="~@/asset/img/course/course_03.png" alt="" width="100%" height="200">
    ...
    export default {
        ...
        methods: {
            ...
            async requestList() { // 获取列表数据
                const res = await requestRelatedCourseList({
                    ...this.pageParam,
            ...
            handleSwitch(id) {
                ...
                // scrollToTop()
                this.redirect(this.pages.detail.course, { query: { id: id } })
            },
        },
        watch: {
            createId() {
