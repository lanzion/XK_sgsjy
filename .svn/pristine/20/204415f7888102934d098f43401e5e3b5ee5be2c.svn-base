...
<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="dataParams.name" @keyup.native.enter="changePage(1)" placeholder="请输入服务名称"></el-input>
                ...
                <el-form :inline="true" :model="dataParams" class="bs-screening_box">
                    <el-form-item label="类目">
                        <el-select v-model="dataParams.category" placeholder="请选择类目" clearable>
                            <el-option v-for="item in serveCategoryDL" :key="item.id" :label="item.name" :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="name" label="服务名称" show-overflow-tooltip>
                <template scope="scope">
                    ...
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/service/detail', query: {id: scope.row.id} }">{{scope.row.name}}</router-link>
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="category" label="类目" width="100" show-overflow-tooltip>
                <template scope="scope">
                    {{scope.row.category | translate(serveCategoryDL, {key: 'id'})}}
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestServiceList, requestServiceAuditDetail, hotService, delService, requestServiceCancelAudit } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestServiceList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            doRecord({data: item} = {data}) {
                requestServiceAuditDetail({serveId: item.id}).then(response => {
                    if (response.data.code == 200) {
            ...
            handleHot({data: item, model} = {data, model}) {
                ...
                if (item.length) {
                    ...
                    if (flag) {
                        this.$message({ message: '只能设置审核通过的服务' })
                    } else {
                        this.$confirm(`此操作将该服务${handleTxt}热门，是否继续？`, `${handleTxt}热门`, {
                            ...
                            hotService(params).then(response => {
                                if (response.data.code == 200) {
                                    item.forEach(x => {
                                        x.isHot = value
                                    })
                    ...
                    this.$message({ message: '请选择要设置的服务' })
                }
            },
            /*
             * 删除
            ...
            doDel({data:item}={data}){
                ...
                if (item.length) {
                    this.$confirm('确定要删除该服务吗?', '提示', {
                        ...
                        console.log('删除服务：', params)
                        delService(params).then((res) => {
                    ...
                    this.$message({message: '请选择要删除的服务'})
                }
            },
            // 撤销审核
            cancelAudit(item) {
                this.$confirm('此操作将撤销该服务的审核，是否继续？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning',
                    customClass: '__custom-msg-box'
                    ...
                    requestServiceCancelAudit(params).then((res) => {
                        if (res.data.code === 200) {
