    ...
    <div class="practice">
        <div class="options-and-search clearfix">
            <div class="options fl">
                ...
                <multi-select :selectData="category" model="activity" :ifAll="false" :clearable="true" style="width:175px;display:inline-block;"></multi-select>
            </div>
            <div class="fr">
        ...
        <div class="list-wrapper">
            <ul>
                <list-row v-for="(item, index) in lists" :item="item" :item-height="156" :key="index">
                    <div slot="content" class="content table-cell">
                        <p>活动名称：
                            <a @click="redirect(pages.detail.activity, { query: {id: item.id} })" :title="item.name" class="blue">{{item.name}}</a>
                        </p>
                        <p>参与时长（天）：{{item.learnLen || 0}}</p>
                        <p>自评等级：<span class="red">{{item.evaluationGrade}}</span></p>
                        <p>自评语：{{item.remark}}</p>
                        <p>实践成果：
                            <a v-for="(result, i) in item.resultList" :key="i" :title="result.name" @click="redirect(pages.detail.works, { query: {id: result.worksId} })" class="achievement-item blue">
                                {{result.name}}
                            </a>
                        </p>
            ...
            <select-range v-if="dialog.isVisible" v-bind="{dialog, curRange, id: curId, handleRequest: requestList, api: '/maker/activityMember/modify'}">
            </select-range>
            <achievement-dialog v-if="achievementDialog.isVisible" v-bind="{curAchievementArr, dialog: achievementDialog, module: 'practice', id: curId, handleRequest: requestList}">
            </achievement-dialog>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            goToEvaluate({ name, memberId, resultList }) { // 进入空间评价系统
                ...
                this.redirect(this.pages.detail.practiceEval, { query: this.$route.query })
            },
