<template>
    <div class="demo">
        <div id="uploader1">
            <div class="queueList">
                <div id="dndArea1" class="placeholders">
            ...
            <div class="statusBar" style="display:none;">
                ...
                <!-- <div class="btns"> -->
                    <div id="filePicker20"></div>
                    <div class="uploadBtns fileBtn">开始上传</div>
                </div>
                <!-- <div class="fileBtn" v-if="!auto">上传</div> -->
                <!-- <div class="progress">
...
export default {
    ...
    mounted() {
        ...
        this.getFileList()
         setTimeout(function () {
          $($($('#uploader1').children()[2]).children()[0]).css({display:'none'})
        }, 10);
    },
    props: {
        projectId: String,
    ...
    methods: {
        init() {
            ...
            // 上传按钮
            this.$upload = this.$wrap.find('.uploadBtns')
            // 没选择文件之前的内容。
            this.$placeHolder = this.$wrap.find('.placeholders')
        ...
        initWebUploader() {
            ...
            WebUploader.Uploader.register({
                ...
                beforeSendFile: function (file) {
                    ...
                    // 根据文件内容来查询MD5
                    that.uploaderFlie.md5File(file, 0, 10 * 1024 * 1024).then(function (val) {
                        // 完成
                        ...
                        $.post((fileUploadAddress + '/file/authnw/checkFileMd5'), { md5: file.md5, name: file.name },
                                function (data) {
        ...
        addFile(file) {
            ...
            if (this.checkIsHasSpecialStr(file.name)) {
                ...
                for (let i = 0; i < this.uploaderFlie.getFiles().length; i++) {
                    if (file.name === this.uploaderFlie.getFiles()[i].name) {
                        ...
                        this.$message({
                            message: '文件名只能包含中文字母数字下划线中划线,并且文件名不能超过50字',
                            type: 'error'
                        })
                    }
                }
            ...
            const $li = $(
                '<li id="' + file.id + '">' +
                    '<div class="lii">' +
                    '<p class="titles fl">' + file.name + '</p>' +
                    '<p class="fl loadTxt">等待上传...</p>' +
                    '</div>' +
                    '<div id="wrapper" class="txtHide"><div id="progressbar"><div id="fill"></div></div></div>' +
                    '</li>')
            ...
            if (file.getStatus() === 'invalid') {
                ...
                // @todo lazyload
                $wrap.text('预览中')
                that.uploaderFlie.makeThumb(file,
                    function (error, src) {
                        let img
                        if (error) {
                            $wrap.text('不能预览')
            ...
            file.on('statuschange', function (cur, prev) {
                if (prev === 'progress') {
                    ...
                    $('#' + file.id).find('#wrapper').removeClass('txtHide')
                ...
                for (let i = 0; i < data.length; i++) {
                    if (i < 11) {
                        ...
                        const $li = $(
                            '<li id="' + data[i].uid + '">' +
                                '<div class="lii">' +
                                '<p class="titles fl">' + data[i].name + '</p>' +
