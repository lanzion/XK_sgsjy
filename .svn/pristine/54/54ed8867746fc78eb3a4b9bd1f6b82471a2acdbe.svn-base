        ...
        <el-dialog title="报名类型" :visible="visible"  size="small" :before-close="closeClear">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
                <el-form-item label="类型名称:" prop="name">
                    <el-col :span="12">
                        <el-input v-model="ruleForm.name" placeholder="请输入学段名称" :maxlength="30"></el-input>
                    </el-col>
                </el-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        { required: true, message: '请输入培训学段名称', trigger: 'change' },
        ...
        methods: {
            del({ data: item } = { data }) {
                ...
                if (item.length) {
                    this.$confirm('确定要删除该学段吗?', '提示', {
                        ...
                        delTrainPeriodByIds(params).then((res) => {
                    ...
                    this.$message({ message: '请选择要删除的学段' })
                }
            },
            edit(val) {
                trainPeriodDetail({ id: val.data.id }).then(res => {
                    res = res.data.entity
            ...
            onSubmit() {
                this.$refs.ruleForm.validate((valid) => {
                    if (valid) {
                        ...
                        if(this.ruleForm.id) {
                            modifyTrainPeriod(data).then (res => {
                                if(res.data.code === 200) {
                                    this.$message({
                                        type: 'success',
                                        message: '修改成功'
            ...
            getData() {
                ...
                requestTrainPeriodList(dataParams, this.pageParam).then(response => {
