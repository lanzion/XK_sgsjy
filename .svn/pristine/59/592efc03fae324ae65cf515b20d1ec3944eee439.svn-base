<template>
  <div class="course-release-add">
    <div class="outline-wrapper">
      <ul class="chapters">
        <li v-for="(chapter, index) in courseData"
            :class="{'has-no-section': chapter.courseHourSectionList.length === 0}" :key="index"
            class="chapter-item">
          <h3 class="chapter">
            {{chapter.name}}
            <span v-if="totalSectionsPerChapter(index)"
                  class="section-num fr">共{{totalSectionsPerChapter(index)}}个课时</span>
          </h3>
          <ul class="sections">
            <li v-for="(section, i) in chapter.courseHourSectionList" :key="i" class="section-item">
              ...
              <operate v-if="section['_canEdit']" :icon-group="['moveup', 'movedown', 'edit', 'delete']" :icon-size="16"
                       @on-delete="deleteSection(chapter.courseHourSectionList, i)"
                       @on-edit="showSectionBox(index, i, true)" @on-move-up="moveUp(chapter.courseHourSectionList, i)"
                       @on-move-down="moveDown(chapter.courseHourSectionList, i)" label="课时"></operate>
            </li>
          </ul>
          <button v-if="!chapter._flag" @click="showSectionBox(index, chapter.courseHourSectionList.length, false)"
                  class="btn-add-section btn">
            添加课时
          </button>
          <el-form v-else :model="sectionForm" :rules="sectionFormRules" ref="sectionForm" label-width="80px"
                   class="add-section-box">
            <el-form-item label="课时类型" prop="sectionType">
              <el-col :span="9">
                <el-select v-model="sectionForm.sectionType" placeholder="请选择课时类型" style="width:100%;">
                  <el-option v-for="item in options.sectionType" :key="item.value" :label="item.label"
            ...
            <el-form-item label="课时名称" prop="sectionName">
              <el-col :span="9">
                <el-input v-model.trim="sectionForm.sectionName" placeholder="请输入课时名称，不超过20字"
            ...
            <el-form-item v-if="sectionForm.sectionType === '1'" label="录播视频" required>
              <el-col :span="24">
                <el-upload action="" :file-list="sectionForm.fileList" :on-remove="deleteAttachment" class="cover-uploader clearfix" :before-upload="UploadFile" accept="video/mp4">
                  ...
                  <div class="tips" style="color:#D00;font-size:12px;text-align:left;" v-if="fileBigSize">上传小于{{ standardFileSize }}M文件</div>
                </el-upload>
              </el-col>
            ...
            <el-form-item v-else-if="sectionForm.sectionType === '2'" label="题目" required>
              <el-col :span="24">
                <a @click="showDialog('selectQuestion')" class="add-section__btn-choose-question" href="javascript:;"
                   title="请选择题目">
                ...
                <ul v-show="questionArr.length" class="selected-question">
                  <li v-for="(question, i) in questionArr" :key="i" class="selected-question-item">
                    ...
                    <operate :icon-group="['delete']" :icon-size="16"
                             @on-delete="deleteSelectedQuestion(i)" label="考题" style="top:0px;right:10px"></operate>
                  </li>
                </ul>
              </el-col>
    ...
    <el-form v-if="addChapterBoxIsVisible" :model="chapterForm" :rules="chapterFormRules" ref="chapterForm"
             label-width="80px" class="add-chapter__box">
