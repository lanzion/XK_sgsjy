<template>
    <div class="backend-content__middle">
        ...
        <el-table class="data-table back-stage-table"  border :data="items" style="width: 100%" ref="multipleTable">
            <el-table-column prop="id" label="编码" width="130" sortable></el-table-column>
        ...
        <el-dialog class="dialog-avatar" :visible.sync="avatarVisible" size="small" title="更换标志">
            <v-avatar-option :img="avatar" :close="close" :save="setAvatar" @changeAvatar="changeAvatar"></v-avatar-option>
        </el-dialog>
        <el-dialog class="dialog-background" :visible.sync="backgroundVisible" size="small" title="更换背景">
            <v-bgcPopup :background="background" :save="setBackground" @changeBackground="changeBackground" :close="close"></v-bgcPopup>
        </el-dialog>
        <el-dialog class="dialog-edit" :visible.sync="editVisible" title="编辑机构信息">
            <v-edit :eduId="curId" :close="close" :updateMsg="updateMsg"></v-edit>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    import avatarOption from '@/components/admin/backstage/avatarOption/avatarOption.vue'
    import adminOperate from '@/components/common/admin_operate.vue'
    import bgcPopup from 'components/admin/backstage/backgroundOption/backgroundOption.vue'
    import editDialog from 'components/admin/backstage/edu/edit_edu.vue'
    ...
    export default {
        ...
        methods: {
             ...
             */
            updateMsg(datas){
                let _datas = Object.assign({}, this.items[this.curIndex], datas)
                this.$set(this.$data.items, this.curIndex, _datas)
            },
            /*
             * 初始化
