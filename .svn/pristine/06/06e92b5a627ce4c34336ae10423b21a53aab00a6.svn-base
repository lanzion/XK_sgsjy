    ...
    <transition-group class="dynamic-list pr" name="dynamic-item" mode="out-in" tag="ul">
      <li class="dynamic-item-detail" v-for="(item, index) in dynamicList.resultData" :key="item.id" :id="item.id" ref="dynamicItem" v-if="filterDelItems([item]).length">
        <section class="dynamic-item-creator table-layout">
          ...
          <router-link v-else tag="img" :to="generateRoute(item)" :src="getFileUrl(item.headPortrait) || faceDefault"
                       alt="" class="dynamic-creator-head circle table-cell is-link"></router-link>
          ...
          <div class="dynamic-btn-group table-cell">
            ...
            <v-del-popover v-if="loginInfo ? item.createId==loginInfo.userInfo.id : false" title="删除" class="delete" :isIcon="false"
                           ...
                           @callback="delDynamicItem(item.id)"></v-del-popover>
          </div>
        </section>
        ...
        <ul class="dynamic-item-commentList" v-if="item.commentList.resultData.length>0"
          ...
          <li class="dynamic-comment-item clearfix">
            ...
            <router-link v-else tag="img" :to="generateRoute(commentItem)"
                         :src="getFileUrl(commentItem.headPortrait) || faceDefault" alt=""
                         class="dynamic-commenter-head circle fl is-link"></router-link>
            <article class="dynamic-comment-detail">
              <a
                v-if="loginInfo && commentItem.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem.identity)"
                class="dynamic-comment-user">{{commentItem.createName}}</a>
              <router-link v-else :to="generateRoute(commentItem)" tag="a" class="dynamic-comment-user">
            ...
            <ul class="dynamic-item-commentList2nd dynamic-item-commentList"
              ...
              <li class="dynamic-comment-item clearfix" v-for="(commentItem2nd, c2) in commentItem.comments.resultData"
                ...
                <img
                  ...
                  class="dynamic-commenter-head circle fl">
                <router-link v-else :to="generateRoute(commentItem2nd)" tag="img"
                             :src="getFileUrl(commentItem2nd.headPortrait) || faceDefault"
                             class="dynamic-commenter-head circle fl is-link"></router-link>
                <article class="dynamic-comment-detail">
                  <a
                    v-if="loginInfo && commentItem2nd.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem2nd.identity)"
                    class="dynamic-comment-user">{{commentItem2nd.createName}}</a>
                  <router-link v-else :to="generateRoute(commentItem2nd)" class="dynamic-comment-user">{{commentItem2nd.createName}}
                  </router-link>：
      ...
      <div key="load-more" class="load-more" v-if="dynamicList.totalNum > dynamicList.resultData.length"
           @click="getDynamicList({pageSize: pager.pageSize, pageNum: dynamicList.nextPageNum, userId, filterType, currentUserId: loginUId})">
  ...
  export default {
    ...
    data () {
      return {
        ...
        identityType: {
          '0': {
            path: '/space/student/dynamic',
            key: ['id', 'uid']
          },
          '1': {
            path: '/space/teacher/dynamic',
            key: ['id', 'uid']
          },
          '2': {
    ...
    created () {
      ...
      window.addEventListener('scroll', () => {
        let dynamicItems = this.$refs.dynamicItem || []
        if (dynamicItems.length) {
          for (let i = 0; i < dynamicItems.length; i++) {
            let _id = this.filterDelItems(this.dynamicList.resultData)[i].id
    ...
    methods: {
      initData () {
        let userId = this.userId = this.isAdmin ? this.loginUId : this.$route.query.uid
        let filterType = this.filterType = this.isAdmin ? 'all' : 'mine'
