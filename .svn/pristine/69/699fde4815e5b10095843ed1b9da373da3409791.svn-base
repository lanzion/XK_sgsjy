        ...
        <div class="about-class bgw clearfix">
            ...
            <div class="class-info fl">
                ...
                <div class="desc clearfix">
                    ...
                    <p class="desc-content">{{claszDatail.classAbstract != null ? claszDatail.classAbstract : ''}}</p>
                </div>
            ...
            <div class="btn-box" v-if="isAdmin">
                <router-link :to="{path:'/claszManage/baseInfo',query:{ id: $route.query.id }}">班级管理</router-link>
            </div>
        </div>
        <nav class="title_nav">
...
export default {
    ...
    data () {
        return {
            navList: [
                {
                    name: '班级主页',
                    path: '/claszSpace/homePage'
                },{
                    name: '班级资源',
                    path: '/claszSpace/resource'
                },{
                    name: '班级作品',
                    path: '/claszSpace/works'
                },{
                    name: '话题吧',
                    path: '/claszSpace/topic'
                },{
                    name: '班级统计',
                    path: '/claszSpace/statis'
                }
            ],
            stuMes: [
    ...
    created(){
        if (this.$route.query.id == undefined && loginInfo) {
            this.$router.replace({ path: this.$route.path, query: { id: baseInfo[userIdentity].classId }})
        };
        if (this.$ls.get('loginInfo')) {
            let studentId = this.$ls.get('loginInfo').userInfo.id
            this.hadLogin = true
                        ...
                        if (this.hadLogin) {
                            if (this.isMember) {
                                ...
                                this.$alert('私密班级，非本班级成员不允许访问!', '温馨提示', {
                                    ...
                                    callback: action => {
                                        window.location.href = `${location.protocol}//${location.host}#/space/clasz`
