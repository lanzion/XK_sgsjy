<template>
    ...
    <div id="region">
        <dl class="region-box category-box" v-for="(title, index) in regionTitle" v-show="regions[index].length" :class="{show}">
            ...
            <dd class="category-title" v-html="title"></dd>
...
export default {
    ...
    data() {
        return {
            regions: [[], [], []],
            regionTitle: [`<i style="margin-right:${this.iWidth}px;">省</i>级：`, `<i style="margin-right:${this.iWidth}px;">地</i>市：`, `<i style="margin-right:${this.iWidth}px;">区</i>县：`],
            show: false,
            moreString: '更多&gt;'
        }
    },
    ...
    methods: {
        ...
        active: function (index, item) {
            ...
            if (level < index) {
                this.regionParams.forEach((x, idx) => {
                    // console.log(x, idx)
                    if (x === undefined && idx < this.regionsLength - 1) {
                        this.regions.splice(idx + 1, 2, [])
                        this.regions.push([], [])
                        this.regions.splice(this.regionsLength)
                    }
                })
