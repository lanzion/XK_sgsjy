<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="作品分布" class="backend-content__module echart table-cell"></v-bar-chart>
        ...
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="workNum" label="作品数" width="120" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestWorksDistribute, requestWorksType, requestWorksDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsWorksEdu', // 作品统计
        components: {
        ...
        methods: {
            // 获取作品分布统计数据
            getDistributeData() {
                requestWorksDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `作品分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，作品总数: ${_data.total || 0}个`,
