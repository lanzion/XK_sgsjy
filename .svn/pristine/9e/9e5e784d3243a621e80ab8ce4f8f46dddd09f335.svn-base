						...
						<div class="content-wrap">
							<p class="clearfix user-box">
								...
								<span class="report fr">
									<v-report v-if="!comment.report && comment.createId !== userId" :report-data="{'url': commentProps.reportApi, params: {'commentId': comment.id, status:'0'}}"></v-report>
								</span>
							</p>
							<p class="content">{{comment.content}}</p>
							<div class="like-reply">
								<div class="option">
									<span class="like" title="点赞" @click="praiseItem(commentList,index)"><i class="icon-btn-thumbsup" :class="{'active':comment.praise}"></i> ({{comment.praiseCount ? comment.praiseCount : 0}}) </span>
								...
								<div class="publish" v-if="show[0] == index && show[1] == null">
									<div class="commment-item-publishbox">
										<textarea placeholder="我来说两句(最多500字)" :maxlength="500" v-model="replyContent" v-on:keyup.enter="replyComment(index,comment.id,comment.id, comment.userName)" style="resize:none"></textarea>
										<div class="btn-box">
											<button @click.prevent="replyComment(index, comment.id, comment.id, comment.userName)">发表</button>
										</div>
									</div>
								</div>
							</div>
									...
									<div class="comment-item-content reply-item-content">
										<section class="clearfix user-box">
											...
											<span class="reply-report fr">
												<v-report v-if="!reply.report && reply.createId !== userId" :report-data="{'url': commentProps.reportApi, params: {'commentId': comment.id, status:'0'}}"></v-report>
											</span>
										</section>
										<article class="content">{{reply.content}}</article>
										<div class="like-reply">
											<div class="option">
												<span class="like" title="点赞" @click="praiseItem(comment[commentProps.replyKey].resultData,i)"><i class="icon-btn-thumbsup" :class="{'active':reply.praise}"></i> ({{reply.praiseCount || 0}}) </span>
        ...
        methods: {
            ...
            async replyComment(index, pId, replyCommentId, replyUserName = null) {
                if (this.$ls.get('loginInfo')) {
                    if (this.replyContent.trim() === '') {
                        ...
                        const params = {
                            [this.commentProps.parmasKey]: this.$route.query[this.commentProps.queryKey],
            ...
            */
            async getEvaluateList() {
                ...
                const res = await this.$http.post(url, { [this.commentProps.parmasKey]: this.$route.query[this.commentProps.queryKey] })
                if (res.data.code === 200) {
            ...
            */
            async getAllReplyList(index, pId) {
                const url = `${this.commentProps.replyListApi}?pageSize=10&pageNum=${this.replyPageNum}`
                const res = await this.$http.post(url, { [this.commentProps.parmasKey]: this.$route.query[this.commentProps.queryKey], parentId: pId })
                if (res.data.code === 200) {
                    if (this.replyPageNum > 1) {
                        ...
                        this.commentList[index].comments.totalNum = res.data.entity.totalNum
                    }
                }
            },
            /*
