<template>
    <div id="feedback" class="backend-content__middle">
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search" placeholder="请输入反馈内容" v-on:keyup.enter="getData"></el-input>
...
<script>
    ...
    export default {
        ...
        methods: {
            getData() {
                let param = {
                    content: this.search, // 反馈标题
                    replyStatus:this.handleStatus.value
                }
                feedbackList( param, this.pageParam ).then(res => {
        ...
        computed: {
            params: function() {
                return Object.assign( {}, this.handleStatus.value,);
            }
        },
        watch: {
            params: {
