...
export default {
    ...
    methods: {
        ...
        getDetail ( id ) {
            if(id){
                this.$router.replace({path:`/studioMemberDetail/articleDetail/${this.$route.params.type}`, query: { id: id, orgId: this.$route.query.orgId} })
                this.comment.list = []
    ...
    watch: {
        '$route.params.type': {
            handler: function (val, oldVal) {
                if(val === '1') {
                    this.crumb.path = '/studioMemberDetail/article'
                    this.crumb.name = '创客研究'
                }else {
                    this.crumb.path = '/studioMemberDetail/result'
