<template>
    <section id="articleDetail">
        <!-- 面包屑 -->
        <el-breadcrumb separator=">" class="navigation container">
            <el-breadcrumb-item @click.native="$router.push({path: `${pages.detail.interest.path}/article`, query: {id: $route.query.orgId}})">文章列表</el-breadcrumb-item>
...
import { articleDetail, articleCommentsAdd, articleCommentsList } from '@/service/interest.js'
...
export default {
    ...
    methods: {
        ...
        getDetail(id) {
            if (id) {
                this.$router.replace({ path: `${this.pages.detail.interest.path}/article/detail`, query: { id, orgId: this.$route.query.orgId } })
                this.comment.list = []
