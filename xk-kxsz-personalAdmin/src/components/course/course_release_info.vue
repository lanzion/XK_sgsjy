    ...
    <div class="admin_project_publish projectPublish course-release-info">
        <el-form ref="stepFirstForm" :model="form" :rules="rules" label-width="100px">
            ...
            <el-form-item label="课程名称" prop="name">
                <el-col :span="9">
                    <el-input :disabled="isPreview" v-model="form.name" placeholder="请输入课程名称，不超过20字" :maxlength="20"></el-input>
                </el-col>
            </el-form-item>
            <!-- 课程封面 -->
            <el-form-item label="课程封面" prop="cover">
                <el-col :span="24">
                    <el-upload :disabled="isPreview" action :show-file-list="false" :before-upload="UploadFile" class="psn-cover-uploader fl">
            ...
            <el-form-item label="学习目标" prop="target">
                <el-col :span="9">
                    <el-input :disabled="isPreview" v-model="form.target" placeholder="请输入课程目标，不超过100字"></el-input>
                </el-col>
            </el-form-item>
            <!-- 课程内容 -->
            <el-form-item label="课程内容" prop="content">
                <el-col :span="20">
                    <el-input :disabled="isPreview" type="textarea" v-model="form.content" placeholder="请输入课程内容，不超过500字" style="height:100px;" :maxlength="500"></el-input>
                </el-col>
            </el-form-item>
            <!-- 课程分类 -->
            <el-form-item label="课程分类" prop="courseType">
                <el-col :span="9">
                    <multi-select :disabled="isPreview" style="width:100%;display:inline-block;" :selectData="form.courseType" model="course"
            ...
            <el-form-item label="相关技术" prop="technologyRelationList">
                <el-col :span="20">
                    <el-select :disabled="isPreview" v-model="form.technologyRelationList" multiple placeholder="请选择相关技术，多选" style="width:100%;" :filterable="true">
            ...
            <el-form-item label="级别" prop="grade">
                <el-col :span="9">
                    <el-select :disabled="isPreview" v-model="form.grade" placeholder="请选择级别，单选" style="width:100%;">
            ...
            <el-form-item label="适合学生" prop="fit">
                <el-col :span="9">
                    <el-checkbox-group v-model="form.fit" :min="1">
                        <el-checkbox :disabled="isPreview" v-for="item in options.fit" :label="item.value" :key="item.value">
                            {{item.name}}
                        </el-checkbox>
                    </el-checkbox-group>
                </el-col>
            ...
            <el-form-item label="课程类型" prop="isFree">
                <el-col :span="9">
                    <el-radio-group v-model="form.isFree">
                        <el-radio v-for="item in options.isFree" :label="item.value" :key="item.value" :disabled="item.disabled || isPreview">
                            {{item.name}}
                        </el-radio>
                    </el-radio-group>
                </el-col>
            ...
            <el-form-item>
                <button @click.prevent="nextStep" class="psn-confirm-btn" type="button">下一步</button>
                <button @click.prevent="$router.go(-1)" class="psn-cancel-btn" type="button">取消</button>
            </el-form-item>
        </el-form>
    </div>
</template>
    ...
    export default {
        ...
        computed: {
            ...mapState('dictionaryCommon', {
                levels(state) {
                    const dicList = (state.courseGradeDicList || {}).dicList || []
                    return dicList
                }
            })
        },
        ...
        methods: {
            ...
            requestCategory() { // 获取相关技术字典数据
                requestDictComb({
                    ...
                    if (res.data.code === 200) {
                        this.options.technologyRelationList = res.data.entity.dicList || []
                    }
                })
            },
            UploadFile(file) {
