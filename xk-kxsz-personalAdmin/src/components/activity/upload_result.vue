<template>
    <el-dialog class="upload-result" title="上传成果" :visible.sync="visible.show">
                        ...
                        <el-col>
                            <el-popover ref="popover" placement="right" width="600" v-model="showPopover" class="res-pop">
                                ...
                                <div class="btn-group btm-btn clearfix">
                                    <button class="psn-cancel-btn fr" @click="cancelSelect">取消</button>
                                    <button class="psn-confirm-btn fr" @click="showPopover = false">确定</button>
                                </div>
                            </el-popover>
                            <span class="psn-upload-btn" v-popover:popover><i class="icon-btn-add-circle"></i> 个人资源库</span>
        ...
        <div class="buttonGroup clearfix">
            ...
            <a class="psn-confirm-btn" @click="uploadResult">确定</a>
        </div>
    </el-dialog>
</template>
            ...
            UploadFile(file) {
                ...
                if (fileSize < (_this.standardFileSize * 1024 * 1024)) {
                    uploader.off('uploadSuccess').on('uploadSuccess', function (files, res) {
                        if (res.code === 200) {
                            _this.form.fileList.push({ name: files.name, url: res.appendInfo.resourceId, status: 'finished' })
                        }
                    })
