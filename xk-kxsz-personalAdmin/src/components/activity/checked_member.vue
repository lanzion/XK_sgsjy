    ...
    <div class="checked_member">
        <ul class="list-box clearfix" v-if="checkedMemberList.length > 0">
              <li class="members fl clearfix" v-for="item in checkedMemberList">
                  <a @click="toPersonDetail(item, 'userId')" class="avatar-box fl">
                      <img :src="getFileUrl(item.headImageUrl) || faceDefault" height="72" width="72">
                  </a>
                  <div class="msg-box">
                      <p><a @click="toPersonDetail(item, 'userId')" class="name">{{item.createName}}</a></p>
    ...
    export default {
        ...
        methods: {
            ...
            *activityId:活动Id
             */
            getAuditedMembers() {
                requestAuditedMember({ activityId: this.$route.query.id, timeStamp: new Date().getTime() }, this.pageParam).then((res) => {
