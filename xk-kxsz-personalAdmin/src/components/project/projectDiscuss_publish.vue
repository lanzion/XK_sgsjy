...
<template>
    <section>
        <div class="discussinitiate-form-select">
            ...
            <i v-if="select.content.length > 2000" class="el-icon-circle-cross" style="color:#23b8ff;"> 超过1000个字符</i>
            <i v-else-if="select.content != ''" class="el-icon-circle-check" style="color:#41C36F;"></i>

            <div class="buttonGroup clearfix">
                <a class="psn-confirm-btn" @click="getData">发表</a>
...
export default {
    ...
    data() {
        return {
            search: { input: '', select: '' },
            select: { options: [{ value: '1', label: '综合讨论区' }, { value: '2', label: '教师答疑区' }], value: '', input: '', content: '' },
            title: ''
        }
    },
    mounted() {
    ...
    methods: {
        getData() {
            ...
            if (this.select.value !== '' && this.select.input !== '' && this.select.content !== '' && this.select.input.length <= 30 && this.select.content.length <= 1000) {
