                    ...
                    <el-table class="data-table" :data="StageStatus.resources" max-height="300" :align="'center'" v-if="StageStatus.resources && StageStatus.resources.length" border>
                        ...
                        <el-table-column label="操作" width="120">
                            <template slot-scope="scope">
                                ...
                                <a href="javascript:;" @click="downloadUrl(scope.row.resourceId, scope.row.name)" style="color:#0391D9;">下载</a>
        ...
        <el-dialog class="detailsOnStage" :visible.sync="completePopup" size="tiny" title="项目阶段完成">
            <div class="box">
                ...
                <div class="buttonGroup clearfix">
                    <button class="psn-cancel-btn" @click="completePopup = false">取消</button>
                    <button class="psn-confirm-btn" @click="sendStage(StageStatus)">确定</button>
                </div>
            </div>
        </el-dialog>
        <!-- 上传成果弹窗 -->
        <el-dialog class="detailsOnStage" :visible.sync="uploadPopup" size="tiny" :before-close="clearFileList" title="上传成果">
            <div class="box">
                ...
                <div class="buttonGroup clearfix" style="margin-top: 30px;">
                    <button class="psn-cancel-btn" @click="clearFileList">取消</button>
                    <button class="psn-confirm-btn" @click="sendResults(StageStatus)">确定</button>
                </div>
            </div>
        </el-dialog>
    </section>
        ...
        UploadFile(file) {
            ...
            if (fileSize < (_this.standardFileSize * 1024 * 1024)) {
                uploader.off('uploadSuccess').on('uploadSuccess', function (files, res) {
                    if (res.code === 200) {
                        _this.fileList.push({ name: files.name, url: res.appendInfo.resourceId, userId: _this.loginUId })
                    }
                })
