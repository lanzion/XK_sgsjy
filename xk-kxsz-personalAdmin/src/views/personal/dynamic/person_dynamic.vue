<template>
  <div class="dynamic" id="person_dynamic">
    <div class="dynamic-type-box" v-if="loginInfo && loginUId == loginInfo.userInfo.id">
    ...
    <transition-group class="dynamic-list pr" name="dynamic-item" mode="out-in" tag="ul">
      <li class="dynamic-item-detail" v-for="(item, index) in dynamicList.resultData" :key="item.id" :id="item.id" ref="dynamicItem" v-if="filterDelItems([item]).length">
        <section class="dynamic-item-creator table-layout">
            <img v-if="loginInfo && item.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(item.identity)"
            :src="getFileUrl(item.headPortrait) || faceDefault" alt="" class="dynamic-creator-head circle table-cell">
            <a v-else @click="toPersonDetail(item, 'createId')">
               <img :src="getFileUrl(item.headPortrait) || faceDefault" class="dynamic-creator-head circle table-cell is-link">
            </a>
            <div class="dynamic-creator-info table-cell">
              <span
                v-if="loginInfo && item.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(item.identity)"
                class="dynamic-creator-name">{{item.createName}}</span>
              <p v-else class="dynamic-creator-name">
                <a @click="toPersonDetail(item, 'createId')" :title="item.createName">
                  {{item.createName}}
                </a>
              </p>
              <time class="dynamic-item-time">{{item.createDate | dateFormat('yyyy-MM-dd HH:mm:ss')}}</time>
              <span
                class="dynamic-item-motion">{{item.title}}</span>
            </div>
            <div class="dynamic-btn-group table-cell">
              <v-report v-if="!item.report && loginInfo && item.createId!==loginInfo.userInfo.id"
                        :report-data="{'url': reportApi, params: {'dynamicId': item.id}}" :item="item" :callback="doReport"></v-report>
              <v-del-popover v-if="loginInfo ? item.createId==loginInfo.userInfo.id : false" title="删除" class="delete" :isIcon="false"
                             :delData="{ url:'maker/dynamic/space/delete', params:{id: item.id}, name:'动态'}"
                             @callback="delDynamicItem(item.id, index)"></v-del-popover>
            </div>
        </section>
        <article class="dynamic-item-content">
          <a target="_blank" @click="toDynamicDetail(item.dynamicType, item.busId)">
        ...
        <section class="dynamic-item-praiseList" v-if="item.praiseCount>0">
          ...
          <template v-for="(praiseItem, p) in item.praiseList.resultData">
            <a
              v-if="loginInfo && praiseItem.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(praiseItem.identity)"
              class="dynamic-item-praiseUser">{{praiseItem.createName}}</a>
            <a v-else tag="a" class="dynamic-item-praiseUser">{{praiseItem.createName}}</a>
        ...
        <ul class="dynamic-item-commentList" v-if="item.commentList.resultData.length>0"
            v-for="(commentItem, c) in item.commentList.resultData" :key="c">
            <li class="dynamic-comment-item clearfix">
              <img
                v-if="loginInfo && commentItem.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem.identity)"
                :src="getFileUrl(commentItem.headPortrait) || faceDefault" alt=""
                class="dynamic-commenter-head circle fl">
              <a v-else @click="toPersonDetail(commentItem, 'createId')">
                    <img :src="getFileUrl(commentItem.headPortrait) || faceDefault" class="dynamic-commenter-head circle fl is-link">
              </a>
              <article class="dynamic-comment-detail">
                <a
                  v-if="loginInfo && commentItem.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem.identity)"
                  class="dynamic-comment-user">{{commentItem.createName}}</a>
              ...
              <a class="load-more-reply" v-if="commentItem.comments.totalNum > commentItem.comments.resultData.length"
                 @click="getReplyList(index, c)">展开更多回复>></a>
              <ul class="dynamic-item-commentList2nd dynamic-item-commentList"
                  v-if="commentItem.comments.resultData.length>0">
                <li class="dynamic-comment-item clearfix" v-for="(commentItem2nd, c2) in commentItem.comments.resultData"
                    :key="c2">
                  <img
                    v-if="loginInfo && commentItem2nd.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem2nd.identity)"
                    :src="getFileUrl(commentItem2nd.headPortrait) || faceDefault" alt=""
                    class="dynamic-commenter-head circle fl">
                  ...
                  <article class="dynamic-comment-detail">
                    <a
                      v-if="loginInfo && commentItem2nd.createId==loginInfo.userInfo.id && !Object.keys(identityType).includes(commentItem2nd.identity)  "
                      class="dynamic-comment-user">{{commentItem2nd.createName}}</a>
              ...
              <div class="dynamic-item-reply-box" v-show="commentItem.isShow">
                <v-send-reply ref="sendReply" @hideInput="hideReplyBox" @submitForm="addComment"
                              :isFocus="commentItem.isShow" :index="[index, c]" :params="replyParams"></v-send-reply>
              </div>
            </li>
        </ul>
        <div class="load-more" v-if="item.commentList.totalNum > item.commentList.resultData.length"
             @click="getCommentsList(index)"><i class="icon-btn-i-more"></i>加载更多
        </div>
      </li>
      <div key="load-more" class="load-more" v-if="dynamicList.totalNum > dynamicList.resultData.length"
           @click="getDynamicList({pageSize: pager.pageSize, pageNum: dynamicList.nextPageNum, userId: loginUId, filterType, currentUserId: loginUId})">
...
<script>
    ...
    import sendReply from '@/components/common/send_reply.vue'
    ...
    export default {
        ...
        methods: {
            ...
            filterTypeChange() {
                ...
                this.getDynamicList({ ...this.pager, userId: this.loginUId, filterType: this.filterType, currentUserId: this.loginUId })
            },
            /*
             * 过滤删除的动态列表
             */
            filterDelItems(items) {
                ...
                return items.filter(item => !_delDynamicList.includes(item.id))
            },
            /*
             * 删除动态
             * id: 动态ID
             */
            delDynamicItem(id, index) {
                this.delDynamicList.push(id)
                this.dynamicList.resultData.splice(index, 1)
            ...
            getCommentsList(index) {
                ...
                dynamicXhr.requestCommentsList({
                    ...
                    const _commentList = Object.assign([], item.commentList.resultData).filter(i => i.updateTime !== undefined)
            ...
            // 跳转动态类型详情
            toDynamicDetail(type, id) {
                if (this.dynamicType[type]) {
                    const detailType = this.dynamicType[type]
                    this.redirect(this.pages.detail[detailType], { query: { id: id } })
                }
            },
            /*
             * 点赞/取消点赞
