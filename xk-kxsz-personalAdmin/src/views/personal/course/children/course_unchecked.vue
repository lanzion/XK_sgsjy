      ...
      <ul>
        <li v-for="(course, index) in courses" :key="index" class="course-item table-layout">
          ...
          <div class="course-info table-cell">
            <h3 class="course-name" v-if="course.auditStatus == '-1' || course.auditStatus == '2' "><router-link :to="{path: 'release', query: {id: course.id}}" class="hover">{{course.name}}</router-link></h3>
            <h3 class="course-name" v-else><router-link :to="{path: 'preview', query: {id: course.id}}" class="hover">{{course.name}}</router-link></h3>
      ...
      async requestListOfTeacher() { // 获取列表数据
        ...
        if (res.data.code === 200) {
          ...
          this.courses.forEach((item) => {
            this.dataTranslation(item.grade, 'courseGrade', { key: 'id' }).then((res1) => {
              item.grade = res1
            })
            this.dataTranslation(item.fit, 'periodKey').then((res2) => {
              item.fit = res2
            })
          })
        }
