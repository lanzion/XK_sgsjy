<template>
    <div :class="{'can-input': canInputAndSubmit}" :style="ISADMIN && adminClass" class="evaluation" ref="evaluation">
        ...
        <div v-if="tableIsVisible" class="my-evaluation dpd15" style="padding-bottom: 26px;">
                        ...
                        <tr class="">
                            ...
                            <td class="">
                                ...
                                <p v-else class="download cell table-content">
                                    ...
                                    <a v-if="downLoadResource && Object.keys(downLoadResource).length" href="javascript:;" @click="downloadUrl(downLoadResource.resourceId, downLoadResource.name)" class="btn-download blue" title="下载">
                                        {{downLoadResource.name}}
                                    </a>
            ...
            <div v-if="btnGroupIsVisible" class="btn-row">
                <button v-if="canInputAndSubmit" @click="submit" class="psn-confirm-btn" type="button">发表
                </button>
                <button @click="$router.go(-1)" class="psn-cancel-btn" type="button">取消</button>
            </div>
        </div>
                ...
                <li v-for="(list, i) in evalResult[2]" :key="i" class="evaluation-item table-layout">
                    ...
                    <div class="content table-cell">
                        ...
                        <p class="attachment">
                            ...
                            <a v-if="list.resource && Object.keys(list.resource).length" href="javascript:;" @click="downloadUrl((list.resource || {}).resourceId, (list.resource || {}).name)"
                                 class="blue" title="下载">
                                {{(list.resource || {}).name}}
                            </a>
                            <a class="btn-download" v-else>暂无附件</a>
    ...
    export default {
        ...
        computed: {
            ...
            btnGroupIsVisible() { // 根据不同场景，控制是否显示按钮组
                ...
                // return this.isAdmin ? type === myEvalType : this.ISADMIN ? type === finalEvalType : type === othersEvalType
                return type === '1'
            },
