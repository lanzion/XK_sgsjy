        ...
        <ul class="project_ul" v-if="listData.length">
            <li class="project_li clearfix" v-for="(item, index) in listData" :key="index">
                ...
                <div class="list_content fr">
                    <h2 class="list_title">
                        <router-link :to="{ path: '/admin/project/detail', query: {'id': item.id}}" v-if='ifStatus'> {{item.name}} </router-link>
                        <router-link :to="{ path: '/admin/project/preview', query: {'id': item.id}}" v-else-if="item.auditStatus === 0 || item.auditStatus === 3"> {{item.name}} </router-link>
                        <router-link :to="{ path: '/admin/project/publish', query: { 'id': item.id } }" v-else>{{item.name}}</router-link>
                    ...
                    <p class="list_p">{{item.content | textOverflow('78')}}</p>
                    <p class="list_type" :class="{ ifStatus: !ifStatus} ">项目分类: {{item.projectTypeName}}
...
import { adminProjectList, projectTeaStuListDel, projectTeaStuSubmitAudit } from '@/service/project.js'
...
export default {
    ...
    methods: {
        ...
        routeChange() {
            ...
            // const list_p = document.querySelectorAll('.list_p')
            if (_route === '/checked') {
                this.ifStatus = true
                this.auditStatus = 1
            } else {
