		...
		<ul class="list-box" v-if="memberList.length > 0">
	  		<li class="member-list clearfix" v-for="item in memberList">
	  			...
	  			<div class="avatar-box fl">
	  				<router-link :to="dynamicPath({identity: item.identity, orgId: item.orgId, userId: item.applyId})">
	  					<img :src="getFileUrl(item.headImageUrl) || faceDefault" height="72" width="73">
	  				</router-link>
	  			</div>
	  			<div class="msg-box fl">
	  				<p>
	  					<router-link :to="dynamicPath({identity: item.identity, orgId: item.orgId, userId: item.applyId})" class="name">{{item.applyName}}</router-link>
