    ...
    methods: {
      initData () { // 初始添加笔记弹窗数据
        if(this.curEditingNote.type === 'edit') { // 编辑笔记
          let {sectionId, content} = this.curEditingNote
      ...
      submit () { // 添加/修改笔记
        this.$refs.addNote.validate(valid => {
          if (valid) { // 验证通过
            ...
            let _label = '添加'
            let id = undefined
            let {sectionId, content} = this.form
            let item = this.sections.find(item => item.id === sectionId)
            let chapterId = item && item.chapterId || ''
