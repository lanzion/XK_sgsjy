<template>
  <article id="courseIntro" class="courseIntroPanel clearfix">
    <img class="courseIntro-prev" title="点击查看原图" @click="visible.show = true" :src="getFileUrl(courseInfo.imgUrl) || listDefault">
    ...
    <div v-else class="courseIntro-summary">
      ...
      <div class="other-info">
        ...
        <span class="courseIntro-creator ellipsis">
          <i class="icon-i-creator"></i>教师：<a @click="linkTo" class="link" href="javascript:;">{{courseInfo.teacher}}</a>
        </span>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async register () { // 报名课程
        ...
        let res = await joinCourse({courseId: this.$route.query.id})
      ...
      linkTo () {
        let {orgId, createId} = this.courseInfo
