		...
		methods:{
			...
			async replyComment(index,pId,replyCommentId,replyUserName = null){
				if(this.$ls.get('loginInfo')){
					if (this.replyContent.trim() === '') {
						...
						let res = await this.$http.post(this.commentProps.addApi,params);
			...
			*/
			async getEvaluateList () {
				let url = `${this.commentProps.getListApi}?pageSize=${this.pageParam.pageSize}&pageNum=${this.pageParam.pageNum}`;
				let res = await this.$http.post(url,{[this.commentProps.parmasKey]:this.id,pId:-1,status:1});
				if (res.data.code === 200) {
			...
			*/
			async getAllReplyList(index,pId) {
				let url = `${this.commentProps.getListApi}?pageSize=10&pageNum=${this.replyPageNum}`;
				let res = await this.$http.post(url,{[this.commentProps.parmasKey]:this.id,pId:pId,status:1})
				if(res.data.code === 200){
