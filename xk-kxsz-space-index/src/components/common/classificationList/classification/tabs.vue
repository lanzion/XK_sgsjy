    ...
    methods: {
        initTab () { // 初始化
          ...
          if (this.router !== undefined && this.router.path !== undefined && this.$route.path.indexOf(this.router.path + '/') >= 0) {
            let _model = this.$route.path.substring(this.router.path.length + 1);
            let _flag = true
        ...
        switchTab (tab, event) {
            let _route = Object.assign({}, this.router)
            _route.path = this.router.path + '/' + this.tabs[tab.index].type;
            this.$router.push(_route);
            this.listenCurTab(tab, event);
        },
        linkTo () {
            var _route = Object.assign({}, this.more);
            var _curTab = this.cur_tab === undefined ? 0 : this.cur_tab;
            if (this.more.path.includes(':')) {
                var _params = _route.path.match(/:\w+/)[0];
