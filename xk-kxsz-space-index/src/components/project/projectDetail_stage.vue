    ...
    <section id="repertoire" class="dpd15">
        ...
        <el-table class="data-table" :data="stageList" style="width: 100%" :align="'center'">
            ...
            <el-table-column prop="phaseStartTime" label="计划时间" width="240">
                <template scope="scope">{{ scope.row.phaseStartTime | dateFormat('yyyy-MM-dd') }} -- {{ scope.row.phaseEndTime | dateFormat('yyyy-MM-dd') }}</template>
            </el-table-column>
            <el-table-column prop="realityStartTime" label="实际时间" width="240">
                <template scope="scope">{{ scope.row.realityStartTime | dateFormat('yyyy-MM-dd') }} -- {{ scope.row.realityEndTime | dateFormat('yyyy-MM-dd') }}</template>
            </el-table-column>
            <el-table-column prop="phaseStatus" label="状态" width="80">
                <template scope="scope"> {{ scope.row.phaseStatus | Stutas }} </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getStageDetail(id, index) {
            this.$http.post('maker/project/phase/detail',
                ...
                if(res.status === 200) {
                    ...
                    let resources = res.data.entity.resources.map( i => ({ name: i.name, url: i.fileid, status:'finished', userId: this.loginUId }) )
                    this.callbackId = id
        ...
        // 上传成果
        sendResults (param) {
            let resourceList = this.fileList.map( i => ({ name: i.name, resourceId: i.url }) )
