    ...
    <section id="projectDetailShare" class="dpd15">
        ...
        <el-table class="data-table" :data="resourceList" style="width: 100%;">
            ...
            <el-table-column prop="fileSize" label="上传时间" width="240" align="center">
                <template scope="scope">{{ scope.row.createDate | dateFormat('yyyy-MM-dd HH:mm:ss')}}</template>
            </el-table-column>
...
import { beforeUploadFile, delUploadMsg } from '@/js/uploadFile.js'
...
export default {
    ...
    methods: {
        ...
        sendResults () {
            ...
            uploadResource(data.concat(fileList2)).then(res => {
                if(res.data.code == 200) {
                    this.getData()
                    this.dialog = false
                    this.fileList = []
