      ...
      <!--<div class="options-and-search clearfix">
        <div class="options fl">
          <multi-select :selectData="category" model="project" :ifAll='true'
                        style="width:130px;display:inline-block;"></multi-select>
          <single-select :selected.sync="selected.technologyId"
                         :single-select-data="{groupCode: 'technicalClassification', 'ifAll': true, placeholder: '选择技术'}"
                         style="width:130px;"></single-select>
          <single-select :selected.sync="selected.grade"
                         :single-select-data="{groupCode: 'courseGrade', 'ifAll': true, placeholder: '选择级别'}"
                         style="width:130px;"></single-select>
        </div>
      ...
      <ul class="clearfix">
        <list-row v-for="(item, index) in lists" :item="item" :item-height="isStudent ? 150 : 118" :key="index">
          <div slot="content" class="content table-cell">
            <figure :style="{'width': coverSize.width + 'px', 'height': coverSize.height + 'px'}"
                    class="pic table-cell pr">
              <img :src="getFileUrl(item.cover) || listDefault"
                   :width="coverSize.width" :height="coverSize.height" alt="">
            </figure>
            <div class="main table-cell">
              ...
              <p>学习成果：
                <router-link v-for="(result, i) in item.studyResultList" :key="i" :title="result.worksName"
                             :to="{path: '/space/works/details', query: {id: result.worksId}}"
    ...
    computed: {
      ...
      dynamicLinks () {
        const source = {
          ...
          3: {name: '实训', link: '/education/train/detail'}
        }
