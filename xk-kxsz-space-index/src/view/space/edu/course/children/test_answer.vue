<template>
  <div class="result-wrapper">
      ...
      <div class="btn-row__result">
        ...
        <router-link :to="{path: '/education/course/detail/outline', query: {id: $route.query.id}}" class="btn">继续练习</router-link>
      </div>
    </div>
</template>
  ...
  export default {
    ...
    methods: {
      async queryLastTestResult () { // 获取上次考试结果列表
        let res = await queryLastTestResult({sectionId: this.sectionId})
        if (res.data.code === 200) {
          let _entity = res.data.entity || {}
          let _list = _entity.examinationPaperList || []
          _list.forEach(item => {
