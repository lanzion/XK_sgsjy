			...
			<el-form ref="form" :model="form" label-width="110px" :rules="rules">
				  	...
				  	<el-form-item label="活动时间" required>
				  		<el-col :span="6">
				  		 	<el-form-item prop="startDate" class="date-box">
					      		<el-date-picker type="datetime" placeholder="开始时间" v-model="form.startDate" :picker-options="startDateOptions" style="width: 100%;" @change="dateChange" clearable></el-date-picker>
					      	</el-form-item>
					    </el-col>
					    <el-col class="line" :span="1" style="text-align:center">—</el-col>
					    <el-col :span="6">
					   		<el-form-item prop="endDate" class="date-box">
					      		<el-date-picker type="datetime" placeholder="结束时间" v-model="form.endDate" :picker-options="endDateOptions" style="width: 100%;" @change="dateChange" clearable></el-date-picker>
					      	</el-form-item>
					    </el-col>
				  	</el-form-item>
			  	</el-row>
                ...
                <el-row>
                    <el-form-item label="报名时间" prop="enrolTime">
                        <el-col :span="11">
                            <el-date-picker type="datetimerange" placeholder="请选择报名时间" v-model="form.enrolTime" :picker-options="stopDateOptions" style="width:100%;" @change="dateChange"></el-date-picker>
                        </el-col>
                    </el-form-item>
                </el-row>
			  	<!-- 活动详情 -->
				...
				<el-form-item>
					<button class="btn submit-btn" @click.prevent="onSubmit(0)">保存</button>
          			<button class="btn submit-btn" @click.prevent="onSubmit(1)">提交审核</button>
         	 		<button class="btn" @click.prevent="$router.push({name:'adminActivity'})">取消</button>
        		</el-form-item>
			</el-form>
		</div>
	</div>
...
<script>
	...
	export default {
		...
		data() {
            ...
            var validateStopDate = (rule, value, callback) => {
	        ...
	        if (this.$route.query.id != undefined) {
	        	requestActivityDetails({id:this.$route.query.id}).then((res)=>{
	        		if(res.status == 200 && res.data.entity != null) {
	        			let detailsData = res.data.entity;
	        			...
	        			this.form.audit = detailsData.audit === 1 ? true : false;
	        			for(let i in detailsData.resourceList){
		...
		methods: {
			dateChange(val) {
				...
				if (startTime && endTime && stopTime) {
					...
					this.startDateOptions = this.endDateOptions = this.stopDateOptions = {
						disabledDate(time) {
							return time.getTime() < Date.now() - 8.64e7
						}
					}
