<template>
	<div class="planPublish theme-red backend-content__middle">
		<el-form ref="form" :model="form" label-width="105px" :rules="rules">
            ...
            <el-row>
                <el-form-item label="策划封面" required style="position:relative;">
                    <el-col :span="14">
                        <el-upload :disabled="isPreview" class="cover-uploader fl" :show-file-list="false" action :before-upload="UploadFile">
		  	...
		  	<el-row>
			  	<el-form-item label="地区" prop="region">
			  		<el-col :span="9">
				    	...
				    	<region-select :disabled="isPreview" :defaultItems="regionArr" @region="selectRegion"></region-select>
				    </el-col>
			  	</el-form-item>
		  	</el-row>
		  	<!-- 详细地址 -->
	        ...
	        <!-- 按钮部分 -->
	        <el-form-item>
	        	<button class="save-btn" @click.prevent="onSubmit(-1)" type="button">保存</button>
	        	<button class="save-btn" @click.prevent="onSubmit(0)" type="button">提交审核</button>
	          	<button class="btn" @click.prevent="$router.push({name:'adminPlan'})">取消</button>
	        </el-form-item>
		</el-form>
	</div>
</template>
...
import regionSelect from 'Common/select/region_pub_select.vue'
	export default {
		...
		data () {
            ...
            var validStartMoney = (rule,value,callback) => {
            ...
            var validEndMoney = (rule,value,callback) => {
		...
		methods: {
			...
			onSubmit(isSubmit){
				let msg = isSubmit == 0 ? '已提交审核' : '保存成功'
				this.$refs['form'].validate((valid)=>{
					if(valid){
						let form = this.form;
						let region = this.form.region.split(',');
						let data = {
							name: form.name,
