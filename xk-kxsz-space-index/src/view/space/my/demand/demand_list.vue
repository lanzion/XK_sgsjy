<template>
    <div id="demandList">
            ...
            <li v-for="(item, index) in listData" class="clearfix">
                ...
                <div class="msg-box fl">
                    ...
                    <p class="msg">
                        ...
                        <span class="icon-btn-i-collection" title="收藏"> {{item.collectionNum ? item.collectionNum : 0}}</span>
        ...
        <v-hookUpWithList v-bind="{ hookUpWithPopup, curHookId, type: '2' }"></v-hookUpWithList>
    </div>
</template>
...
import { demandPendingList, demandSubmitAudit, demandDel } from '@/service/demand.js'
import operate from 'components/common/operate.vue'
import verifyRecordDialog from 'components/my/verify_record_dialog.vue'
import hookUpWithList from 'View/space/my/hookupWith/hookupWithList.vue'
    export default {
        name: 'serviceList',
        ...
        components: {
            ...
            operate,
            verifyRecordDialog,
            'v-hookUpWithList': hookUpWithList
        },
        data () {
            return {
        ...
        methods: {
            ...
            // 请求列表数据
            getListData () {
                demandPendingList({ name: this.search, category: this.selected, auditStatusTeaOrStu: this.auditStatus, createId: this.$ls.get('loginUId') }, this.param).then(res => {
