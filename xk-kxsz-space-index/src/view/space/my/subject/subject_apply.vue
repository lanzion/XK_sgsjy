		...
		<el-form ref="form" :model="form" label-width="163px" :rules="rules">
			...
			<el-row>
				<el-form-item label="附件">
				    <el-col :span="12">
				    	<el-upload class="upload-demo" :disabled="isPreview" action :file-list="fileList" :on-remove="handleRemove" :before-upload="UploadFile">
			...
			<el-form-item v-if="!isPreview">
				<button class="bs-search-button i-form-button" @click.prevent="applySubmit(0)">保存</button>
				<button class="bs-search-button i-form-button" @click.prevent="applySubmit(1)">提交审核</button>
	          	<button class="bs-search-button i-form-button is-plain" @click.prevent="$router.push(routeLink)">取消</button>
	        </el-form-item>
		</el-form>
	</div>
</template>
...
<script>
	...
	import selectParticipator from 'Common/module/subject/select_participator.vue';
	import { requestDictComb } from '@/service/common.js';
	import { applySubject,modifySubject,requestSubjectDetails,requestSchoolTeachers } from '@/service/subject.js';
	...
	export default {
		...
		data () {
			var validNum = (rule, value, callback) => {
                let regular = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
			...
			return {
	            ...
	            rules: {
	                ...
	                funding: [
	                    ...
	                    { validator: validNum },
	                    // { type: 'number', message: '经费数额必须为数字值', trigger: 'change' }
	                ],
	                projectDate:[
			...
			*id:this.$route.query.subjectId
			...
			getSubjectDetails() {
				requestSubjectDetails({ id: this.$route.query.subjectId }).then((res) => {
					...
					for (let i in _data.attList) {
        				this.fileList.push({ name: _data.attList[i].name, size: attList[i].fileSize, url: _data.attList[i].resourceId, status: 'finished' })
        			}
				})
			},
			/*
	       	...
	       	UploadFile (file, fileList) {
	            ...
	            if (fileSize < (_this.standardFileSize * 1024 * 1024)) {
	                uploader.off('uploadSuccess').on('uploadSuccess', function (file, res) {
	                    if (res.code === 200) {
	                        _this.fileList.push({name: file.name, size: file.size, url: res.appendInfo.resourceId});
	                    }
	                })
