<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search.name" @keyup.native.enter="changePage(1)" placeholder="请输入众创空间名称"></el-input>
            ...
            <el-table-column prop="account" label="账号" sortable show-overflow-tooltip>
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/base/users/mult/detail', query: {id: scope.row.id} }">{{scope.row.account}}</router-link>
                    </span>
                </template>
            </el-table-column>
        ...
        <el-dialog class="dialog-audit" :visible.sync="auditVisible" size="tiny" title="众创空间审核">
            <div class="radio-box">
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestBaseMult(this.params).then(response => {
            ...
            /*
             * 推荐
             * { "relationIds": ["众创空间ID"] , "recommendType": 1 }
             */
            doRecommend({data: item} = {data}) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过且未锁定的众创空间' })
                    } else {
                        item.forEach(x => {
                    ...
                    this.$message({ message: '请选择要推荐的众创空间' });
                }
            },
            // 判断是否可推荐
            isRecommend(item) {
            ...
            lockOperate({data: item, model} = {data, model}){
                ...
                item = JSON.stringify(item) === '{}' ? this.checkedItems : [item]
                if (item.length) {
                    this.$confirm(`此操作将${text}该众创空间，是否继续？`, text, {
                    ...
                    this.$message({ message: `请选择要${text}的众创空间` })
                }
            },
            /*
             * 审核
            ...
            popupAudit({data = {}, index} = {data, index}){
                if (JSON.stringify(data) === '{}') {
                    if (this.checkedItems.length > 0) {
                        ...
                        this.showMessage('info', '请选择要审核的众创空间')
                    }
                } else {
                    this.auditVisible = true
                    this.curItem = data
            ...
            popupPwd({data = {}, index} = {data, index}) {
                if (JSON.stringify(data) === '{}') {
                    if (this.checkedItems.length > 0) {
                        ...
                        this.showMessage('info', '请勾选要修改密码的众创空间')
                    }
                } else {
                    this.pwdVisible = true
                    this.curItem = data
            ...
            doDel({data:item}={data}){
                ...
                if (item.length) {
                    ...
                    this.$message({message: '请选择要删除的众创空间'})
                }
            },
            closeResetPwdDialog() {
