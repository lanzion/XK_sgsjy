<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search.name" @keyup.native.enter="changePage(1)" placeholder="请输入投资机构名称"></el-input>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="account" label="营业执照">
                <template scope="scope">
                    <a :href="getFileUrl(scope.row.certiPic)" target="_blank">
                        <img :src="getFileUrl(scope.row.certiPic)" />
                    </a>
                </template>
            </el-table-column>
            <el-table-column prop="account" label="账号" sortable show-overflow-tooltip>
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/base/users/investorg/detail', query: {id: scope.row.id} }">{{scope.row.account}}</router-link>
                    </span>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="机构名称"></el-table-column>
            <el-table-column prop="certiNum" label="工商注册号码" width="160" sortable show-overflow-tooltip></el-table-column>
        ...
        <el-dialog class="dialog-audit" :visible.sync="auditVisible" size="tiny" title="投资机构审核">
            <div class="radio-box">
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestBaseInvestOrg(this.params).then(response => {
            ...
            /*
             * 推荐
             * { "relationIds": ["投资机构ID"] , "recommendType": 1 }
             */
            doRecommend({data: item} = {data}) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过且未锁定的投资机构' })
                    } else {
                        item.forEach(x => {
                    ...
                    this.$message({ message: '请选择要推荐的投资机构' });
                }
            },
            // 判断是否可推荐
            isRecommend(item) {
            ...
            lockOperate({data: item, model} = {data, model}){
                ...
                item = JSON.stringify(item) === '{}' ? this.checkedItems : [item]
                if (item.length) {
                    this.$confirm(`此操作将${text}该投资机构，是否继续？`, text, {
                    ...
                    this.$message({ message: `请选择要${text}的投资机构` })
                }
            },
            /*
             * 审核
            ...
            popupAudit({data = {}, index} = {data, index}){
                if (JSON.stringify(data) === '{}') {
                    if (this.checkedItems.length > 0) {
                        ...
                        this.showMessage('info', '请选择要审核的投资机构')
                    }
                } else {
                    this.auditVisible = true
                    this.curItem = data
            ...
            popupPwd({data = {}, index} = {data, index}) {
                if (JSON.stringify(data) === '{}') {
                    if (this.checkedItems.length > 0) {
                        ...
                        this.showMessage('info', '请勾选要修改密码的投资机构')
                    }
                } else {
                    this.pwdVisible = true
                    this.curItem = data
            ...
            doDel({data:item}={data}){
                ...
                if (item.length) {
                    this.$confirm('确定要删除该投资机构吗?', '提示', {
                        ...
                        delBaseInvestOrg(params).then((res) => {
                            if (res.data.code === 200) {
                    ...
                    this.$message({message: '请选择要删除的投资机构'})
                }
            },
            closeResetPwdDialog() {
