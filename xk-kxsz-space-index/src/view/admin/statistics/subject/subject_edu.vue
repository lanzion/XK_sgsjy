<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="课题分布" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="typeDatas" title="课题领域" centerTitle="课题(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="subjectNum" label="课题数" width="120" show-overflow-tooltip></el-table-column>
                <el-table-column prop="sujbectResultNum" label="课题成果数" width="120" show-overflow-tooltip></el-table-column>
                <el-table-column label="领域">
                    <el-table-column v-for="(type, t) in detailColumn.type" :label="type" :key="t">
                        <template scope="scope">
                            {{scope.row.researchFieldList[t].num}}
                        </template>
                    </el-table-column>
                </el-table-column>
...
<script>
    ...
    import { requestSubjectDistribute, requestSubjectType, requestSubjectDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsSubjectEdu', // 课题统计
        components: {
        ...
        methods: {
            // 获取课题分布统计数据
            getDistributeData() {
                requestSubjectDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `课题分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，课题总数: ${_data.total || 0}个`,
                ...
                requestSubjectDetail(this.detailParams).then(response => {
                    if (response.data.code == 200) {
                        ...
                        if (_data.resultData.length) {
                            _data.resultData[0].researchFieldList.forEach(x => {
                                _typeColumn.push(x.lineName)
                            })
                            _data.resultData[0].statusList.forEach(x => {
