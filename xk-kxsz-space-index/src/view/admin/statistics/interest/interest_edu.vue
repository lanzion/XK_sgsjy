<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="兴趣组分布" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="typeDatas" title="兴趣组分类" centerTitle="兴趣组(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="interestsNum" label="兴趣组数" width="120" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestInterestDistribute, requestInterestType, requestInterestDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsInterestEdu', // 兴趣组统计
        components: {
        ...
        methods: {
            // 获取兴趣组分布统计数据
            getDistributeData() {
                requestInterestDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `兴趣组分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，兴趣组总数: ${_data.total || 0}个`,
