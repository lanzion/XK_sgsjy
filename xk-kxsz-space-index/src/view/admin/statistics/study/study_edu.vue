<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="学习分布" class="backend-content__module echart table-cell"></v-bar-chart>
        ...
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data.sync="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="studyNum" label="学习数" width="120" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column label="人群">
                    <el-table-column v-for="(status, s) in detailColumn.status" :label="status" :key="s">
                        <template scope="scope">
                            {{scope.row.crowdList[s].countNum}}
                        </template>
                    </el-table-column>
                </el-table-column>
...
<script>
    ...
    import { requestStudyDistribute, requestStudyType, requestStudyDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsStudyEdu', // 学习统计
        components: {
        ...
        methods: {
            // 获取学习分布统计数据
            getDistributeData() {
                requestStudyDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `学习分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，学习总数: ${_data.total || 0}个`,
                ...
                requestStudyDetail(this.detailParams).then(response => {
                    if (response.data.code == 200) {
                        ...
                        if (_data.resultData.length) {
                            ...
                            _data.resultData[0].crowdList.forEach(x => {
