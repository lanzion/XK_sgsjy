<template>
	<section id="service_detail" class="backend-content__middle">
		<dl class="detail-item clearfix">
			<dt style="line-height: 100px;">服务封面：</dt>
			<dd class="img-box">
			...
			<dd>
				<ul v-if="detail.resourceList.length">
					<li v-for="(item,index) in detail.resourceList">
						<a href="javaScript:;" style="color: #2797ed;">{{item.name}}</a>
					</li>
				</ul>
...
<script>
	...
	import { serviceDetail } from '@/service/service.js'
	import { requestServiceAuditDetail, requestServiceAudit } from '@/service/admin_space.js'  // 列表数据接口
	...
	export default {
		...
		methods:{
            ...
            //获取审核记录
            popupAudit(){
                requestServiceAuditDetail({serveId:this.$route.query.id}).then((res) => {
                    if(res.data.code === 200){
