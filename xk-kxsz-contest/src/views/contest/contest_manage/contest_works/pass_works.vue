    ...
    <div class="content-box">
        <section class="option-box clearfix">
            <div class="btnBox">
                ...
                <button
                    class="export-btn"
                    :class="{'score_btn':!scoreBtnFlag}"
                    @click="automaticAwards"
            ...
            <div class="works-count clearfix" v-if="identity != matchDetail.matchGrade">
                <span class="tip fl">
                    <i class="icon-form-tip"></i>待上报作品数
                </span>
            ...
            <div v-show="screeningDropdown" class="bs-screening_wrap">
                ...
                >
                    ...
                    <el-form-item label="当前赛程">
                        <el-select
                            v-model="search.phaseId"
                            placeholder="请选择参赛形式"
                            filterable
                            clearable
                        >
                            <el-option
                                v-for="item in phaseDL"
                                :key="item.id"
                                :label="item.name"
                                :value="item.id"
                            ></el-option>
                    ...
                    <el-form-item label="组别">
                        <el-select
                            v-model="search.groupDisDetailId"
                            placeholder="请选择组别"
                            filterable
                            clearable
                        >
                    ...
                    <el-form-item label="学科">
                        <el-select
                            v-model="search.subjectDisDetailId"
                            placeholder="请选择学科"
                            filterable
                            clearable
                        >
                    ...
                    <el-form-item label="是否上报">
                        <el-select v-model="search.isReport" placeholder="请选择" filterable clearable>
                            <el-option
                ...
                <div class="bs-screening-btn">
                    <button type="button" @click="getWorksList">确定</button> |
        ...
        >
            ...
            <el-table-column prop="name" label="作品名称" show-overflow-tooltip>
                <template slot-scope="scope">
                    <router-link
                        class="a-btn"
                        :to="{path: '/contest/manage/works/audit', query: { id: $route.query.id, worksId: scope.row.id, type: $route.query.type } }"
            ...
            <el-table-column label="操作" width="235" fixed="right">
                <template slot-scope="scope">
                    ...
                    <span v-if="identity != 9&&identity==6">|</span>
                    <router-link
                        v-if="identity != 9&&identity==6"
                        :to="{ path: '/contest/manage/works/reviewRecord', query: { id: $route.query.id, worksId: scope.row.id, type: $route.query.type } }"
                        class="option-btn"
                    >评审记录</router-link>|
                    <a
                        href="javaScript:;"
                        v-if="identity != 9&&identity==6"
                        class="option-btn"
                        @click="openCompeteDialog(scope.row.id)"
                    >参赛记录</a>
                    <span v-if="identity != 9&&identity==6">|</span>
                    <a
                        v-if="scope.row.isPrize === '1'"
                        class="option-btn"
                        @click="openAwardRecord(scope.row.id)"
                    >已评奖</a>
                    <a
                        href="javaScript:;"
                        class="option-btn"
                        v-else
                        @click="openAwardDialog(scope.row.phaseId,scope.row.phaseName,scope.row.id,scope.row.createId)"
                    >评奖</a>
                    <div class="dis_ib" v-if="identity != matchDetail.matchGrade">
                        ...
                        <a
                            href="javaScript:;"
                            class="option-btn"
                            v-if="scope.row.isReport == null || scope.row.isReport == 0"
                            @click="worksOperate(1, scope.row.id, scope.row.phaseId, scope.$index)"
                        >上报</a>
                        <a
                            href="javaScript:;"
                            class="option-btn"
                            v-else
                            @click="worksOperate(0, scope.row.id, scope.row.phaseId, scope.$index)"
                        >撤回</a>
                    </div>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        // 获取赛程阶段数据
        getPhaseList() {
            requestWorksPhaseList({ matchId: this.$route.query.id }).then(
                res => {
                    if (res.data.code === 200) {
        ...
        getAwarRecord(worksId) {
            ...
            requestAwardRecord(params).then(res => {
                if (res.data.code === 200) {
                    this.awardRecord =
                        res.data.appendInfo.matchAwardsRecordsList
                }
            })
        },
        // 打开参赛获奖弹窗
        ...
        automaticAwards() {
            ...
            automaticAwards({ matchId: matchId, phaseId: this.phaseId }).then(
                res => {
                    let data = res.data
                    console.log(data)
                    if (data.code == 200) {
