<template>
	<div id="works_detail" class="w100" v-bind:class="{'container': $route.path.includes('worksDetail')}">
		...
		<div class="detail-wrap bgw">
    			...
    			<section class="works-info clearfix">
    				<div class="fl info-box bxs_bb" v-for="(info, index) in worksInfo">
    					<dl class="clearfix info-item" v-for="(item, index) in info.info">
    						...
    						<dl class="item-name fl bxs_bb ellipsis" v-else-if="item.name === '参赛人群'">{{actorType[detail.actorType] || worksDetail.identity}}</dl>
    		    ...
    		    <section class="works-detail">
    				...
    				<dl class="detail-file clearfix">
    					...
    					<dd class="fl file" v-if="worksDetail.attachmentList != null && worksDetail.attachmentList.length">
    						...
    						<a class="c_blue" v-for="(file, index) in worksDetail.attachmentList" href="javascript:;" @click="downloadUrl(file.resourceId, file.name )">{{file.name}}</a><br>
    					</dd>
            ...
            <section class="praise-operate">
                <div class="praise-box">
                    <span class="icon-btn icon-btn-thumbsup" :title="praiseTitle" :class="{active: ifPrasise}" @click="praiseOperate"></span>
                    <!-- <p class="praise-txt" v-if="ifPrasise">已点赞</p> -->
                    <p class="praise-txt">赞</p>
                </div>
                <div class="stati-data">
                    <span class="msg-praise">点赞：<i class="c_orange">{{praiseNum || 0}}</i>人</span>
                    <span class="msg-browse" v-if="Number($route.query.type) === 1">浏览：<i class="c_orange">{{worksDetail.browseNum || 0}}</i>人</span>
                    <span class="msg-browse" v-else>浏览：<i class="c_orange">{{offlineBrowseNum || 0}}</i>人</span>
                    <span class="msg-score" v-if="Number($route.query.type) === 1">评分：<i class="c_orange">{{worksDetail.score || 0}}</i></span>
                </div>
            </section>
		</div>
...
<script>
    ...
    import { requestWorksDetail, requestEvalIndex, countBrowseNum, countOfflineBrowseNum, judgePraise, commonStatistics, worksPraise, worksCancelPraise } from '@/service/works.js'
    ...
    export default {
        ...
        created() {
            if (this.$route.path === '/contest/review/audited/detail/review') {
                ...
                this.breadTxt = '经我评审'
            } else if (this.$route.path === '/contest/review/pending/detail/review') {
        ...
        mounted() {
            ...
            if (this.$ls.get('loginUId')) {
                judgePraise({ busId: this.$route.query.worksId, userId: this.$ls.get('loginUId') }).then((res) => {
            ...
            // 竞赛-作品-计算浏览数量
            const handle = Number(this.$route.query.type) === 1 ? countBrowseNum : countOfflineBrowseNum
        ...
        methods: {
                ...
                requestWorksDetail({ id: this.$route.query.worksId }).then((res) => {
                    if (res.data.code === 200) {
                        ...
                        if (this.$route.path.includes('review')) {
                            // 获取评审指标
                            requestEvalIndex({ projectId: this.worksDetail.projectId }).then((response) => {
                                if (response.data.code === 200) {
                                    this.indexList = response.data.entity.resultData
                                    for (let i = 0; i < this.indexList.length; i++) {
                                        this.$set(this.indexList[i], 'scoreInput', 0)
                                    }
                                }
                            })
                        }
            ...
            downloadName(name){
                return this.worksDetail.groupDisDetailName+'_' + this.worksDetail.projectName + '_' + this.worksDetail.schoolName + '_' + this.worksDetail.userName + '_' + name;
            }
        }
    }
</script>
