...
<template>
    <section class="myPlay_upload bgc_w">
        ...
        <el-form ref="form" :model="form" :rules="rule" label-width="120px" class="form">
            <el-form-item
                label="作者名称"
                prop="checkedUserList"
                v-if="activityDetail.actorType === '1'&&!isStudent"
            >
                <el-row>
                    <el-col :span="6">
                        <el-input v-model="userName" placeholder="请输入作者姓名"></el-input>
                    </el-col>
                    <el-col :span="12">
                        <el-button class="group_button sure ml15" @click="getUserList">查找</el-button>
                        <span class="fileTips" style="padding-left: 15px;">注：查找之后请点击头像加号选择参赛作者</span>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="12">
                        <el-checkbox-group v-model="form.checkedUserList" v-show="false"></el-checkbox-group>
                        <el-tag
                            :key="index"
                            v-for="(tag, index) in form.checkedUserList"
                            :closable="true"
                            :close-transition="false"
                            @close="delTag('User', index)"
                            class="tag"
                        >{{tag.name}}</el-tag>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="22">
                        <transition-group
                            ...
                            mode="out-in"
                        >
                            <li class="item-item" v-for="(i, k) in userList" :key="k">
                                <section class="item-item-detail" @click="addUser(i)">
                                    <img
                                        class="item-img fl"
                                        :src="getFileUrl(i.face) || faceDefault"
                                    />
                                    <div class="item-msg">
                                        <p class="name">{{ i.name }}</p>
                                        <p class="name">{{ i.account }}</p>
                                    </div>
                                    <template>
                                        <i class="item-item-handle i-success-bg"></i>
                                        <i class="item-item-handle icon-btn-follow2"></i>
                                    </template>
                                </section>
                            </li>
                        </transition-group>
            ...
            <el-form-item label="作品名称" prop="name">
                ...
                <el-col :span="4">
                    <button
                        type="button"
                        class="btn_selectWork"
                        @click="addResourcePopop = true;"
                    >选择</button>
                </el-col>
            </el-form-item>
            ...
            <el-form-item
                label="指导老师"
                v-if="activityDetail.actorType === '1'"
                prop="checkedTeacherList"
            >
                <el-row>
                    <el-col :span="6">
                    ...
                    <el-col :span="12">
                        <el-button
                            ...
                            @click="getPersonList('teacher')"
                        >查找</el-button>
                        <span class="fileTips" style="padding-left: 15px;">注：查找之后请点击头像加号选择指导老师</span>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="12">
                    ...
                    <el-col :span="22">
                        ...
                        >
                            <li class="item-item" v-for="(i, k) in teacherList" :key="k">
                                <section class="item-item-detail" @click="add('Teacher', i)">
                                    <img
                                        class="item-img fl"
                                        :src="getFileUrl(i.face) || faceDefault"
                                    />
            ...
            <el-form-item label="参赛成员" prop="checkedStudentList" v-if="form.actorWay === '2'">
                <el-row>
                    ...
                    <el-col :span="12">
                        ...
                        >查找</el-button>
                        <span class="fileTips" style="padding-left: 15px;">注：查找之后请点击头像加号选择参赛成员</span>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="12">
                    ...
                    <el-col :span="22">
                        ...
                        >
                            <li class="item-item" v-for="(i, k) in studentList" :key="k">
                                <section class="item-item-detail" @click="add('Student', i)">
                                    <img
                                        class="item-img fl"
                                        :src="getFileUrl(i.face) || faceDefault"
                                    />
...
export default {
            ...
            if (res.data.response) {
                if (res.data.response.status) {
                    if (
                        ...
                        const va = {
                            resourceId:
                                da.response.data.group +
                                '/' +
                                da.response.data.remoteFileName,
                            name: da.file.name,
                            size: da.file.size,
                            documentId: ''
                        }
    ...
    computed: {
        activityDetail() {
            return this.$store.state.matchDetail.detail
        }
    },
    watch: {
        detail: n => {
    ...
    methods: {
        ...
        submit() {
            ...
            this.$refs.form.validate(valid => {
                if (valid && flag) {
                    ...
                    param.checkedStudentList = param.checkedStudentList.map(
                        i => ({
                            userId: i.id,
                            relationType: '2'
                        })
                    )
                    param.checkedTeacherList = param.checkedTeacherList.map(
                        i => ({
                            userId: i.id,
                            relationType: '1'
                        })
                    ...
                    //     }
                    // }
                    if (param.checkedUserList.length) {
                        param.userId = param.checkedUserList[0].id
                    } else {
                        let baseInfo = localStorage.getItem('baseInfo')
                        let userId = JSON.parse(baseInfo).baseInfo.userId
                        param.userId = userId
                    }
                    delete param.checkedStudentList
                    delete param.checkedTeacherList
                    delete param.checkedUserList
        ...
        add(model, item) {
            if (
                !this.form[`checked${model}List`].some(
                    i => i.id === item.userId
                )
            ) {
                ...
                const str =
                    model === 'Teacher' ? '已添加此指导老师' : '已添加此成员'
                this.$message({ message: str })
            }
        },
        addUser(item) {
            this.form.checkedUserList = [{ id: item.userId, name: item.name }]
        },
        ...
        getPersonList(module, isMember = false) {
            ...
            if (name) {
                ...
                //         ? requestTeacherList
                //         : requestStudentList
                // let reqName = module === 'teacher' ? 'teacherName' : 'name'
                let request, reqName
                if (
                    !isMember ||
                    (module === 'teacher' &&
                        this.activityDetail.actorType === '2')
                ) {
                    request = requestTeacherList
                    reqName = 'teacherName'
                } else {
                    request = requestStudentList
                    reqName = 'name'
                }
                let param = { auditStatus: 1 }
                param[reqName] = name
        ...
        getUserList() {
            requestStudentList({ name: this.userName }).then(res => {
                if (res.data.code === 200) {
                    this.userList = res.data.entity.resultData
                }
            })
        },
        // 上传作品图片
