...
import {
    ...
    userIdentity,
    removeUserInfo,
} from 'Asset/js/getUserInfo'
import encrypt from 'jsencrypt'
import Axios from 'Asset/js/http'
import Cookies from 'Asset/js/cookies'
import redirect from 'Asset/js/redirectCallback'
import { pages, httpConfig } from '@/assets/js/common-config'
...
const actions = {
    ...
    async login({ dispatch, commit, state }, option) {
        ...
        return handler(params).then((response) => {
            ...
            if (_code > 200 && _code <= 205) { // 登录失败返回失败信息
                ...
                // 存储登录信息
                Cookies.set({ token: _data.token }, { 'max-age': _data.timeOut, domain: httpConfig.domain })
                Cookies.set({ uid: _data.userInfo.id }, { 'max-age': _data.timeOut, domain: httpConfig.domain })
    ...
    async getPermission({ commit }) {
        return requestPermission().then((response) => {
            if (response.data.code === 200) {
                const _data = response.data.appendInfo.userRight
                commit('toggleLoginModal', false)
                localStorage.setItem('permission', JSON.stringify(_data))
                return _data
            }
        })
    },
