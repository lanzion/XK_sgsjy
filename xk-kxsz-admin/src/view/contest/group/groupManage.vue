<template>
    <div class="backend-content__middle">
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search.name" @keyup.native.enter="changePage(1)" placeholder="请输入申报组别"></el-input>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="name" label="申报组别" width="280" sortable show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestContestGroup, delContestGroup } from '@/service/admin_contest-offline.js'
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestContestGroup(this.params).then(response => {
            ...
            doDel({data: item} = {data}){
                ...
                if (item.length) {
                    this.$confirm('此操作将删除该申报组别，是否继续？', '提示', {
                        ...
                        delContestGroup(params).then((res) => {
                    ...
                    this.$message({ message: '请选择要删除的申报组别' })
                }
            },
            // 改变分页
            changePage(val) {
