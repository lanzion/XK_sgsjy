        ...
        <el-form ref="form" :model="form" label-width="120px" :rules="rules">
            ...
            <el-row>
                <el-form-item label="机构名称" prop="name">
                    <el-col :span="8">
                        <el-input v-model="form.name" placeholder="请输入机构名称" :disabled="isReadOnly"></el-input>
                    </el-col>
                </el-form-item>
            </el-row>
            <!-- 工商注册号码 -->
...
import { addBaseInvestOrg, requestInvestOrgDetail, modifyBaseInvestOrg } from '@/service/admin_base.js'
import region from '@/components/common/select/region_pub_select.vue'
export default {
    name: 'investOrgManageAdd',
    ...
    data() {
        ...
        const validateCertiNum = (rule, value, callback) => {
            ...
            if (value === '') {
                ...
                if (base[index] == lastCode) {
                    ...
                    callback(new Error('请输入正确格式的工商注册号码1'))
                }
            }
        }
        const validateUserName = (rule, value, callback) => {
            ...
            if (value === '') {
                callback(new Error(`请输入投资机构全称`))
            } else if (!regular.test(value)) {
                callback(new Error(`您输入的投资机构全称格式不正确`))
            } else {
                callback()
            }
        }
        ...
        return {
            ...
            rules: {
                ...
                certiNum: [
                    ...
                    { required: true, message: '请输入工商注册号码', trigger: 'blur' }
                ],
                certiPic: [
    ...
    methods: {
        ...
        //获取机构详情
        getDetail(){
            requestInvestOrgDetail({id:this.form.id}).then((res) => {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    let flag = this.$route.query.id ? true : false,
                        handle = flag ? modifyBaseInvestOrg : addBaseInvestOrg,
