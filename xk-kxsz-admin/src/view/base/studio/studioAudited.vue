<template>
    <div>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="createName" label="教师姓名" show-overflow-tooltip>
                <template scope="scope">
            ...
            <el-table-column prop="auditStatus" label="状态" sortable width="80">
                <template scope="scope">
...
<script>
    ...
    import { requestBaseStudio, requestStudioAuditRecord, cancelAuditStudio } from '@/service/admin_base.js'
    ...
    export default {
        name: 'studio_audited',
        mixins: [subPermission, setPage, recommend],
        components: {
        ...
        methods: {
            ...
            /*
             * 推荐
             * { "relationIds": ["导师工作室ID"] , "recommendType": 2 }
             */
            doRecommend({data: item} = {data}) {
                let params = { relationIds: [], recommendType: 17 },
                    flag   = false;
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过,未锁定和未推荐的导师工作室' })
                    } else {
                        this.recommendOperate(item, params, 17)
                    }
                } else {
                    this.$message({message: '请选择要推荐的导师工作室'});
                }
            },
            //取消推荐
            cancelRecommend({ data: item } = { data }) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '您选择的导师工作室中包含了未推荐的导师工作室' })
                    } else {
                        this.recommendOperate(item, params, 17, true)
                    }
                } else {
                    this.$message({message: '请选择要取消推荐的导师工作室'});
                }
            },
            // 撤销审核
            cancelAudit(item) {
