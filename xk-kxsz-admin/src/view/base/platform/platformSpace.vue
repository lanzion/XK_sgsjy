<template>
    <div class="backend-content__middle">
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="search.spaceName" @keyup.native.enter="changePage(1)" placeholder="请输入空间名称"></el-input>
            ...
            <div v-show="screeningDropdown" class="bs-screening_wrap">
                <el-form :inline="true" :model="search" class="bs-screening_box">
                    ...
                    <el-form-item label="空间类型">
                        <el-select v-model="search.spaceType" placeholder="请选择空间类型" filterable clearable>
                            <el-option v-for="item in spaceTypeDL" :key="item.id" :label="item.name" :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="spaceName" label="空间名称" show-overflow-tooltip>
            ...
            <el-table-column prop="spaceType" label="空间类型" show-overflow-tooltip>
                <template scope="scope">
                    {{scope.row.spaceType | translate(spaceTypeDL)}}
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestPlatformSpaceList, switchStatus } from '@/service/admin_base.js'
    ...
    export default {
        ...
        computed: {
            ...mapState('dictionaryCommon', {
                ...
                spaceTypeDL (state) {
                    const dicList = (state.spaceTypeDicList || {}).dicList || []
                    return dicList
                }
            }),
            params: function() {
        ...
        methods:{
             ...
             */
            getData() {
                requestPlatformSpaceList(this.params, this.pageParam).then(response => {
            ...
            /*
             * 推荐
             * { "relationIds": ["空间ID"] , "recommendType": 2 }
             */
            doRecommend({data: item} = {data}) {
                let params = { relationIds: [], recommendType: 15 },
                    flag   = false;
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '只能推荐审核通过,未锁定和未推荐的空间' })
                    } else {
                        this.recommendOperate(item, params, 15)
                    }
                } else {
                    this.$message({message: '请选择要推荐的空间'});
                }
            },
            //取消推荐
            cancelRecommend({ data: item } = { data }) {
                ...
                if (item.length) {
                    ...
                    if(flag) {
                        this.$message({ message: '您选择的空间中包含了未推荐的空间' })
                    } else {
                        this.recommendOperate(item, params, 15, true)
                    }
                } else {
                    this.$message({message: '请选择要取消推荐的空间'});
                }
            },
            // 判断是否可推荐
            isRecommend(item) {
            ...
            lockOperate({data: item, model} = {data, model}){
                ...
                item = JSON.stringify(item) === '{}' ? this.checkedItems : [item]
                if (item.length) {
                    this.$confirm(`此操作将${text}该空间，是否继续？`, text, {
                    ...
                    this.$message({ message: `请选择要${text}的空间` })
                }
            }
        }
    }
