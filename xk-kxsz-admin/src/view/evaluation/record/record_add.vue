<template>
  <div :class="{'can-input': canInputAndSubmit}" :style="ISADMIN && adminClass"
       class="evaluation" ref="evaluation">
    ...
    <div v-if="tableIsVisible" class="my-evaluation" style="padding-bottom: 26px;">
      <el-table :data="evalResult[evalType].evaluationList || items" ref="multipleTable" border
        ...
        <el-table-column :label="evalType === '1' && '自评' || '评分'" width="128">
          <template scope="scope">
            ...
            <template v-else>
              <el-input-number v-if="canInputAndSubmit" v-model.number="evaluationList[scope.$index].score" class="score-input is-required"
                        type="number" :min="0" :max="scope.row.score" placeholder="请输入"></el-input-number>
      ...
      <div
        class="el-table data-table back-stage-table el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
        ...
        <div class="el-table__body-wrapper">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width: 100%;">
            ...
            <tr class="">
              ...
              <td class="">
                <p class="cell table-content red">
                  {{ISADMIN && evalType === '3' && finalLevel || evalResult[evalType].evaluationGrade}}</p>
              </td>
            </tr>
            <tr class="">
              <td class="table-label">
              ...
              <td class="">
                <el-input v-if="canInputAndSubmit" v-model.trim="comment" :maxlength="80" class="cell table-content is-required"
              ...
              <td class="">
                <ul class="cell table-content clearfix">
                  <li v-for="(val, i) in achievementList" :key="i" class="works-item blue">
                    <!-- <span v-if="isAdmin">{{val.worksName}}</span> -->
                    <a @click="gotoWorksDetailPage(val.worksId)" :title="val.worksName" class="blue" href="javascript:;">{{val.worksName}}</a>
                  </li>
                </ul>
              </td>
            </tr>
...
<script>
  ...
  import {
    ...
    queryEvalStatus,
    submitFinalEval
  } from '@/service/admin_evaluation.js'
    ...
    computed: {
      ...
      btnGroupIsVisible () { // 根据不同场景，控制是否显示按钮组
        ...
        return this.isAdmin
          ? type === myEvalType
          : this.ISADMIN
            ? type === finalEvalType : type === othersEvalType
      },
...
//      _isFinal && this.requestFinalList()
    ...
    methods: {
      async requestQuotaList () { // 获取指标列表
        ...
        const res = await requestQuotaList({target, createId, createDate})
      ...
      async queryEvalResult () { // 查询评价结果
        ...
        const res = await queryEvalResult({busId})
      ...
      UploadFile (file, fileList) {
        ...
        if(fileSize < (this.standardFileSize * 1024 * 1024) ) {
          ...
          _this.fileList.splice(0, 1)
          _this.$message({ message: `文件大于${standardFileSize}` })
