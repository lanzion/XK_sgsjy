		...
		<div class="video-wrapper">
			<div class="container clearfix">
				<div class="video-box fl">
					<div class="wrapper">
						<video id="video" class="video-js" v-if="videoSrcArr[curVideoSrcIndex]" ref="video" :src="getFileUrl(videoSrcArr[curVideoSrcIndex].url, {filetype: 'video'})" @click="togglePlay" @ended="handleEnd" @play.native="play" @pause.native="pause" @error="handleError" :key="curVideoSrcIndex" webkit-playsinline="true" x-webkit-airplay="allow" preload="auto" controls="controls" width="892" height="504"></video>
					...
					<div class="bottom">
						...
						<a @click="$message('已经上报了故障，谢谢您的配合')" class="report-error" href="javascript:;">
...
<script>
	...
	import courseDetail from '@/components/space/video/videoCourseDetail.vue'
	import outline from '@/components/space/video/outline.vue'
	import note from '@/components/space/video/note.vue'
		...
		watch: {
			...
			videoSrcArr (val) {
				...
				if (!sectionId && firstSrcSectionId) {
					...
					this.errMessageBox && this.errMessageBox.close()
					this.$router.replace({ path: '', query: { id, sectionId: firstSrcSectionId } })
				}
			}
		}
	}
