<template>
    <div class="admin_infoPublish backend-content__middle">
        <el-form ref="form" :model="form" label-width="80px" :rules="rules">
            ...
            <el-row>
                <el-form-item label="申报时间"  prop="limit">
                    <el-col :span="9">
                        <el-date-picker v-model="form.limit" type="daterange" placeholder="选择日期范围" :picker-options="dateOptions" range-separator=" / " style="width:100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
            </el-row>
            <el-row>
        ...
        <el-dialog title="添加专家" :visible.sync="addExpertsPopup" size="tiny">
            <div class="func" v-if="$ls.get('userIdentity')=='school'">
                <el-input placeholder="搜索" icon="search" v-model="teacherParam.name" @click="getTeacherList(true)" @keyup.native.enter="getTeacherList(true)"></el-input>
            </div>
            <el-row :gutter="20" v-else>
                <div class="func col clearfix">
                    <el-col :span="16">
                        <v-region-select @region="changeRegion" :clearable="true"></v-region-select>
                    </el-col>
                    <el-col :span="8">
                        <el-input placeholder="搜索" icon="search" v-model="teacherParam.name" @click="getTeacherList(true)" @keyup.native.enter="getTeacherList(true)"></el-input>
                    </el-col>
            ...
            UploadFile (file, fileList) {
                ...
                if (fileSize < (_this.standardFileSize * 1024 * 1024)) {
                    uploader.off('uploadSuccess').on('uploadSuccess', function (file, res) {
                        if (res.code === 200) {
                            _this.fileList.push({name: file.name, size: fileSize, url: res.appendInfo.resourceId, status:'finished'})
                        }
                    })
            ...
            onSubmit (formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        let resourceList = form.resourceList.map(i => ({ resourceId : i.id, name: i.name, fileSize: i.fileSize}) )
                        let attList = this.fileList.map(i => ({ resourceId : i.url, name: i.name, fileSize: i.size}) )
                        let data = {
                            title: form.name,
