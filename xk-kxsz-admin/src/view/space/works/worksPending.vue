...
<template>
    <div>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="workName" label="作品名称" show-overflow-tooltip>
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/works/detail', query: {id: scope.row.workId} }">{{scope.row.workName}}</router-link>
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="identity" label="作者身份" show-overflow-tooltip>
                <template scope="scope">
                    {{scope.row.identity | translate(identityDL)}}
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestWorksAuditList, requestWorksAudit, requestWorksCancelAudit } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                requestWorksAuditList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            // 撤销审核
            cancelAudit(item) {
                this.$confirm('此操作将撤销该作品的审核，是否继续？', '提示', {
                    ...
                    requestWorksCancelAudit(params).then((res) => {
            ...
            auditSubmit (item) {
                ...
                requestWorksAudit(this.AuditParams).then(res => {
                    if (res.data.code == 200) {
