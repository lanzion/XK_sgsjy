    ...
    <div>
        ...
        <el-dialog title="审核记录" :visible.sync="auditRecordVisible" class="audit-record-dialog">
            <el-table class="data-table back-stage-table" border :data="recordDatas">
                ...
                <el-table-column property="auditStatus" label="审核结果" :width="operateWidth" v-if="listBtnGroup.length">
                    <template scope="scope">
                        <el-tag :class="['__color-'+auditStatusStyle[scope.row.auditStatus]]">
                            {{scope.row.auditStatus | translate(auditStatusDL)}}
                        </el-tag>
...
<script>
    ...
    import { requestProjectList, requestProjectAuditDetail, hotProject, requestProjectCancelAudit, delProject } from '@/service/admin_space.js' // 列表数据接口
...
// 配置按钮组件
    export default {
        ...
        methods: {
            ...
            // 撤销审核
            cancelAudit(item) {
                this.$confirm('此操作将撤销该项目的审核，是否继续？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning',
                    customClass: '__custom-msg-box'
                    ...
                    requestProjectCancelAudit(params).then((res) => {
                        if (res.data.code === 200) {
