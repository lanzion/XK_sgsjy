...
<template>
    <div>
        <section class="option-box clearfix">
            <el-input class="fl bs-search-input" v-model="dataParams.storyName" @keyup.native.enter="changePage(1)" placeholder="请输入故事名称"></el-input>
        ...
        <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable" @selection-change="changeSelection">
            ...
            <el-table-column prop="storyName" label="故事标题" show-overflow-tooltip width="200">
                <template scope="scope">
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path:'/space/story/detail', query: {id: scope.row.id} }">{{scope.row.storyName}}</router-link>
                    </span>
                </template>
            </el-table-column>
...
<script>
    ...
    import { requestStoryAuditList, requestStoryAudit, requestStoryCancelAudit } from '@/service/admin_space.js'  // 列表数据接口
    ...
    export default {
        ...
        methods: {
             ...
             */
            getData() {
                requestStoryAuditList(this.dataParams, this.pageParam).then(response => {
                    if (response.data.code == 200) {
            ...
            resetSearchForm() {
                ...
                this.dataParams = Object.assign({}, this._dataParams, { name: this.dataParams.storyName })
