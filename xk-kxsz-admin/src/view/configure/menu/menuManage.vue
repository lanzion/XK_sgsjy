        ...
        <div class="menu-content table-cell menu-cell">
            <section class="backend-content__middle submit-form">
                ...
                <el-form ref="form" label-position="right" label-width="114px" :model="form" :rules="rules">
                    ...
                    <el-row>
                        <el-form-item label="groupPermiss" prop="groupPermiss">
                            <el-col :span="10">
                    ...
                    <el-row v-if="permissTypeList.some(x => x.position && x.id===permissType)">
    ...
    export default {
        ...
        data() {
            return {
                ...
                permissTypeList: [
                    ...
                    { id: '3', label: '弹窗', position: true },
                    { id: '4', label: '分类', position: true },
                    { id: '5', label: '外链' },
                    { id: '6', label: '开关' }
                ],
                permissPositionList: [
        ...
        methods: {
            ...
            renderContent(h, { node, data, store }) {
                ...
                return (
                    <span>
                        <span class={'el-tree-node__label' + (data.status=='0' ? ' unActive' : '')}>
                            {node.label}
                            <span class='el-tree-node__label unActive'>
                                {node.data.permissType ? '［' + this.permissTypeList[node.data.permissType.split('-')[0]-1].label + '］' : '［菜单］'}
                            </span>
                        </span>
            ...
            getPermissType() {
                let _pt = this.permissType,
                    ...
                    result = null
                if (_pt !== '' && _pp !== '' && this.permissTypeList.some(x => x.id == _pt && x.position)) {
                    result = [_pt, _pp].join('-')
                } else {
                    result = _pt
                }
