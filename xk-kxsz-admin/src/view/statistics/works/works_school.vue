<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-tab-chart :datas="distributeDatas" title="作品分布" :legend="['年级', '班级', '人群']" class="backend-content__module echart table-cell"></v-bar-tab-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="typeDatas" title="作品分类" centerTitle="作品(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="num" label="作品数" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestWorksDistributeByGrade, requestWorksDistributeByClasz, requestWorksDistributeByCrowd, requestWorksType, requestWorksDetail } from '@/service/statis_school.js'

    export default {
        name: 'statisticsWorksSchool', // 作品统计
        components: {
        ...
        methods: {
            // 获取作品分类统计数据
            getTypeData() {
                requestWorksType(this.typeParams).then(response => {
            ...
            // 获取作品分布统计数据
            getDistributeData() {
                requestWorksDistributeByGrade(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.data || {}
                        let _result = {
                ...
                requestWorksDistributeByClasz(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.data || {}
                        let _result = {
                ...
                requestWorksDistributeByCrowd(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.data || {}
                        let _result = {
                            subtext: `作品人群分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，作品总数: ${_data.total || 0}个`,
                            xAxis: _data.dataAxis || [],
                            data: _data.data || []
                        }
            ...
            // 获取作品详细数据
            getDetailData() {
                requestWorksDetail(this.detailParams).then(response => {
                    if (response.data.code == 200) {
