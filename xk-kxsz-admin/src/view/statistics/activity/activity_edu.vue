<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="活动分布" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="typeDatas" title="活动分类" centerTitle="活动(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="activityNum" label="活动数" width="120" show-overflow-tooltip></el-table-column>
                <el-table-column prop="activityResultNum" label="活动成果数" width="120" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestActivityDistribute, requestActivityType, requestActivityDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsActivityEdu', // 活动统计
        components: {
        ...
        methods: {
            // 获取活动分布统计数据
            getDistributeData() {
                requestActivityDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `活动分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，活动总数: ${_data.total || 0}个`,
