<template>
    <div>
        ...
        <div class="table-layout">
            <v-bar-chart :datas="distributeDatas" title="项目分布" class="backend-content__module echart table-cell"></v-bar-chart>
            <div class="small-module table-cell echart">
                <v-pie-d-chart :datas="typeDatas" title="项目分类" centerTitle="项目(个)" class="backend-content__module"></v-pie-d-chart>
            </div>
        </div>
        <section class="backend-content__middle">
            ...
            <el-table class="data-table back-stage-table" border :data="items" style="width: 100%" ref="multipleTable">
                ...
                <el-table-column prop="projectNum" label="项目数" width="120" show-overflow-tooltip></el-table-column>
                <el-table-column prop="projectResultNum" label="项目成果数" width="120" show-overflow-tooltip></el-table-column>
...
<script>
    ...
    import { requestProjectDistribute, requestProjectType, requestProjectDetail } from '@/service/statis_edu.js'

    export default {
        name: 'statisticsProjectEdu', // 项目统计
        components: {
        ...
        methods: {
            // 获取项目分布统计数据
            getDistributeData() {
                requestProjectDistribute(this.typeParams).then(response => {
                    if (response.data.code == 200) {
                        let _data = response.data.appendInfo.item || {}
                        let _result = {
                            subtext: `项目分布: ${_data.dataAxis ? _data.dataAxis.length : 0}，项目总数: ${_data.total || 0}个`,
