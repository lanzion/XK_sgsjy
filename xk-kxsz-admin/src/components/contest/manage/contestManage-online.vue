            ...
            <el-table-column prop="name" label="竞赛主题" show-overflow-tooltip>
                <template scope="scope">
                    ...
                    <span class="resource-name">
                        <router-link tag="a" class="item-detail" :to="{ path: `/contest-${$route.params.type}/match/detail`, query: {id: scope.row.id, type: 1} }">{{scope.row.title}}</router-link>
                    </span>
                </template>
            </el-table-column>
            <el-table-column prop="projectListString" label="赛事项目" show-overflow-tooltip></el-table-column>
            <el-table-column prop="singleMaxNum" label="最多作品限制" width="100" show-overflow-tooltip>
                <template scope="scope">
            ...
            <el-table-column label="操作" fixed="right" :width="operateWidth" v-if="listBtnGroup.length">
                <template scope="scope">
                    <v-admin-operate :items="listBtnGroup" :data="scope.row" :index="scope.$index" v-bind="{
                        ...
                        del: { callback: doDel, isActive: isDel },
    ...
    export default {
        ...
        computed: {
            ...mapState('dictionaryCommon', {
                ...
                actorTypeDL(state) {
                    const dicList = (state.actorTypeDicList || {}).dicList || []
                    return dicList
                },
                matchGradeDL(state) {
                    const dicList = (state.matchGradeDicList || {}).dicList || []
                    return dicList
                }
            }),
            params: function () {
        ...
        methods: {
            ...
            getData() {
                ...
                handler(this.params).then((response) => {
                    if (response.data.code === 200) {
                        const _data = (response.data.entity || {}).resultData || []
            ...
            doRecommend({ data: item } = { data }) {
                const params = { relationIds: [], recommendType: 18 }
