    ...
    <div class="studioArticle container clearfix">
        ...
        <div class="content-right bgw fr">
            <div class="search-box clearfix">
                <button type="button" v-if="hasPermissions" class="fr publish" @click="pubVisible = true">发布研究</button>
            ...
            <ul class="res-list" v-if="items.length > 0">
                <li class="list-item clearfix" v-for="(item,index) in items">
                    <div class="fl title-box">
                        ...
                        <router-link :to="{ path:`${pages.detail.studio.path}/articleDetail/${typeNum}`, query: { id: item.id, orgId: $route.query.id } }" class="fl title">{{item.title}}</router-link>
                    </div>
        ...
        <!-- 发布修改 -->
        <el-dialog title="发布研究" :visible.sync="pubVisible" size="small" :before-close="closePopup" lock-scroll>
            <el-form :model="form" :rules="rules" ref="form" label-width="80px">
                ...
                <!-- 按钮部分 -->
                <el-form-item>
                    <el-col :span="22" style="text-align: right;" class="group_btn">
                        <button class="btn cancel" @click.prevent="closePopup">取消</button>
                        <button class="btn sure" @click.prevent="onSubmit">确定</button>
                    </el-col>
                </el-form-item>
            </el-form>
        </el-dialog>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 获取列表
            getResList() {
                const params = Object.assign({}, this.search, { studioId: this.$route.query.id, studioType: this.typeNum })
