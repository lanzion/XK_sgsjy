    ...
    <div class="interest">
        ...
        <div class="options-and-search clearfix">
            <div class="options fl">
                <multi-select :selectData="category" model="interest" :ifAll="false" :clearable="true" style="width:175px;display:inline-block;"></multi-select>
            ...
            <ul>
                <list-row v-for="(item, index) in lists" :item="item" :key="index">
                    <h3 slot="title" class="name ellipsis">
                        <a @click="toInterestDetail(item.id)" :title="item.name" class="link">
                            {{item.name || '暂无'}}
                        </a>
                    </h3>
                    <p class="other-info">
                    ...
                    <div slot="operate" class="operate pa">
                        <a @click="toInterestDetail(item.id)" class="btn">
                            进入兴趣组
                        </a>
                    </div>
                </list-row>
    ...
    export default {
        ...
        methods: {
            async requestList() { // 获取列表数据
                const res = await requestMyInterestList({
                    ...this.selected,
