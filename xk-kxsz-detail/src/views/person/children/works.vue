    ...
    <div class="works">
        ...
        <div class="options-and-search clearfix">
            <div class="options fl">
                <multi-select :selectData="category" model="works" :ifAll="false" :clearable="true" style="width:175px;display:inline-block;"></multi-select>
            ...
            <ul>
                <list-row v-for="(item, index) in lists" :item="item" module="works" :key="index">
                    <h3 slot="title" class="name ellipsis">
                        <a @click="redirect(pages.detail.works, { query: { id: item.id } })" :title="item.workName" class="link">
                            {{item.workName || '暂无'}}
                        </a>
                    </h3>
                    <p class="other-info">
    ...
    export default {
        ...
        methods: {
            async requestList() { // 获取列表数据
                const res = await requestFrontendWorksList({
                    ...this.selected,
            ...
            toDetailPage({ workName, id, createTime }) { // 进入空间评价系统
                ...
                this.redirect(this.pages.detail.works, { query: { id: id } })
            },
            changePage(val) {
