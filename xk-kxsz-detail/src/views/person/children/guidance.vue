    ...
    <div class="guidance">
        ...
        <div class="options-and-search clearfix">
            <div class="options fl">
                <multi-select :selectData="category" :key="selectedType" :model="selectedType === '1' && 'course' ||    selectedType === '2' && 'project' || 'activity'" :ifAll='false' style="width:130px;display:inline-block;" :clearable='true'></multi-select>
                <single-select v-if="selectedType === '1' || selectedType === '2'" :selected.sync="selected.technologyId" :single-select-data="{groupCode: 'technicalClassification', placeholder: '选择技术'}" style="width:130px;"></single-select>
                <single-select v-if="selectedType === '1'" :selected.sync="selected.grade" :key="selectedType" :single-select-data="{groupCode: 'courseGrade', placeholder: '选择级别'}" style="width:130px;"></single-select>
                <single-select v-if="selectedType === '2'" :selected.sync="selected.applyScopeId" :key="selectedType" :single-select-data="{groupCode: 'periodKey', placeholder: '选择学段'}" style="width:130px;"></single-select>
                <template v-if="selectedType === '3'">
                    <single-select :selected.sync="selected.enrollStatus" :key="selectedType" :single-select-data="{groupCode: 'activityRegister', placeholder: '报名状态'}" style="width:130px;"></single-select>
                    <single-select :selected.sync="selected.status" :key="selectedType" :single-select-data="{groupCode: 'activeState', placeholder: '活动状态'}"style="width:130px;"></single-select>
                </template>
            </div>
        ...
        <div class="list-wrapper">
            <ul>
                <list-row v-for="(item, index) in lists" :item="item" :key="index">
                    <figcaption v-if="selectedType === '3'" slot="status" :class="{ing: item.isApply !== -1 && item.isApply !== 6 && Number(item.startDate) < Number(nowTime), end: item.isApply === 6}" class="status pa">
                        <span class="txt pa">{{handleActivityStatus(item.startDate,item.isApply)}}</span>
                    </figcaption>
                    <h3 slot="title" class="name ellipsis">
                        <a @click="redirect(pages.detail[detailPath], { query: { id: item.id } })" :title="item.name || item.title" class="link">
                    ...
                    <div slot="operate" class="operate pa">
                        ...
                        <template v-else>
                            <a @click="redirect(pages.detail[detailPath], { query: { id: item.id } })" class="btn">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            options() {
                const { coursesNum = 0, projectsNum = 0, activityNum = 0 } = this.relatedData
                return [
            ...
            async requestList(page) { // 获取列表数据
                ...
                if (res.data.code === 200) {
                    ...
                    this.lists.forEach((item) => {
                        this.dataTranslation(item.grade, 'courseGrade', { key: 'id' }).then((x) => {
                            item.grade = x || '无'
                        })
                        this.dataTranslation(item.applyScopeId, 'periodKey', { key: 'id' }).then((x) => {
                            item.applyScopeId = x || '无'
                        })
                    })
                }
            },
