            ...
            getMemberList() {
                requestClaszMember({ classId: this.claszId }).then((res) => {
                    if (res.data.code === 200 && res.data.entity !== null) {
                        ...
                        this.getDictComb('technicalClassification').then(() => {
                            _data.forEach((item) => {
                                // 任教转译
                                dataTranslation(item.interest, 'technicalClassification', { key: 'id' }).then((x) => {
                                    item.interest = x
                                })
                            })
                        })
                ...
                this.$confirm('此操作将设置该学生为管理员，是否继续？', '提示', {
                    ...
                    setClaszAdmin(params).then((res) => {
                        if (res.data.code === 200) {
                            ...
                            this.$router.push({ path: this.pages.detail.clasz.path, query: { id: this.$route.query.id } })
                        } else {
