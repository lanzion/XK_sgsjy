<template>
    <section class="topic_detail">
        ...
        <nav class="navigation container">
            <router-link :to="{path: routePath, query: {id: $route.query.id}}"> 全部话题 </router-link> > 话题详情
        </nav>
...
<script>
    ...
    import reply from 'components/clasz/topic/reply.vue'
    ...
    export default {
        ...
        created() {
            ...
            if (path.includes('clasz')) {
                this.routePath = `${this.pages.detail.clasz.path}/topic`
            } else if (path.includes('studio')) {
                this.routePath = `${this.pages.detail.studio.path}/topic`
            } else {
                this.routePath = `${this.pages.detail.interest.path}/topic`
            }
        },
        mounted() {
            this.getTopicDetail()
