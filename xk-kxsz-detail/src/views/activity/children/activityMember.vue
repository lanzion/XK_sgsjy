        ...
        <div id="activityMember">
            <ul class="list clearfix" v-if="memberList.length > 0">
                <li class="fl" v-for="item in memberList" :style="{width:width}">
                    <div class="img-box">
                        <router-link :to="dynamicPath(item)" target="_blank"><img :src="getFileUrl(item.headImageUrl) || faceDefault" width="72" height="72"></router-link>
                    </div>
                    <p class="name-box">
                        <router-link :to="dynamicPath(item)" target="_blank">{{item.createName}}</router-link>
                    </p>
                </li>
            </ul>
