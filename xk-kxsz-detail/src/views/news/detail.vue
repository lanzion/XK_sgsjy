    ...
    <div class="infoDetail" >
        ...
        <div class="bgc_w container">
            ...
            <ul class="attList" v-if="detail.portalNoticeDetailResource && detail.portalNoticeDetailResource.length">
                ...
                <li v-for="(i, k) in detail.portalNoticeDetailResource" class="attList_li"> <a @click="downloadUrl(i.resourceId, i.resourceId.split('/')[i.resourceId.split('/').length - 1] )" class="btn" title="下载">{{ i.resourceName }} </a></li>
            </ul>
        </div>
     </div>
</template>
