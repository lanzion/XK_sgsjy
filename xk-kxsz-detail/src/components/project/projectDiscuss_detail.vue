...
<template>
    <section id="projectDiscuss_detail">
        ...
        <div class="comments">
            <h3 class="comments_title">共{{totalNum || 0}}条回复</h3>
...
export default {
    ...
    methods: {
        addDiscuss: function (event) {
            ...
            const parentObj = target.parent().parent()
            const lastObj = parentObj.find('.reply')
        ...
        reply() {
            ...
            const param = { projectId: query.id, discussType: query.discussType, content: this.text, pId: this.param.id, secondId: this.param.id }
