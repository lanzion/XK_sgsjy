<template>
    <div class="note-wrapper">
        ...
        <ul class="note-container">
            <li v-for="(note, index) in notes" :key="index" @mouseenter="mouseEnterItem(index)" @mouseleave="mouseLeaveItem(index)" class="note-item table-layout">
                <edit-note-area v-if="(textAreaIsVisible === true) && (curNoteItem === index)" :note-txt="note.content" @on-save-note="saveNote" @on-cancel-edit="hideTextArea" class="table-cell" style="width:100%;"></edit-note-area>
                <div v-else class="note-content table-cell">
                    ...
                    <div v-show="curNoteItem === index" class="operate">
                        <a @click="textAreaIsVisible = true" href="javascript:;" class="btn-edit" title="编辑">
                            <i class="icon-btn-edit2"></i>
                        </a>
                        <a @click="deleteItem(note.id)" href="javascript:;" class="btn-delete" title="删除">
    ...
    export default {
        ...
        methods: {
            async requestNoteList() { // 获取笔记列表
                const res = await requestNoteList({ ...this.pageParam, courseId: this.id, sectionId: this.sectionId })
            ...
            addItem(options = {}) { // 添加/修改笔记
                const isEditing = options.type === 'edit'
                const handler = isEditing && editNote || addNote
                const _note = isEditing && options.txt || this.myNote
                const _label = isEditing && '修改' || '添加'
                const id = isEditing ? this.notes[this.curNoteItem].id : undefined
                const _item = this.videoSrcArr.find(item => item.sectionId === this.sectionId)
                const videoCurTime = this.videoEl && +(this.videoEl.currentTime.toFixed(2)) || 0 // 视频当前播放时间点
    ...
    .note-container {
        ...
        .note-item {
            ...
            .icon {
                ...
                height: 26px;
                margin-right: 6px;
                background: url("~@/assets/img/course/suspend_small.png") no-repeat;
