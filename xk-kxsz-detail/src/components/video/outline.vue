    ...
    <div class="outline-wrapper">
        <ul class="chapters">
            <li v-for="(chapter, index) in courseOutline" :key="index" class="chapter-item">
                ...
                <ul class="sections">
                    <li v-for="(section, i) in chapter.courseHourSectionList" @click="startStudying(section)" :class="{'cur-section': $route.query.sectionId === section.id}" :key="i" class="section-item">
    ...
    export default {
        ...
        methods: {
            startStudying(section) {
                const option = { id: this.id, sectionId: section.id }
                if (section.sectionType === '1') {
                    this.$router.replace({ path: '', query: option })
                } else if (section.sectionType === '2') {
                    this.$router.push({ path: `${this.pages.detail.course.path}/test`, query: option })
                }
            }
        }
    }
