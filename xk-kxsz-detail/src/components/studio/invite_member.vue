    ...
    methods: {
        ...
        selectData(type) {
            ...
            for (let i = 0; i < _data.length; i++) {
                this[_selector].push(_data[i].name)
                this[_selected].push(_data[i].userId)
            }
        ...
        send() {
            if (localStorage.getItem('loginInfo')) {
                ...
                const text = this.content + `<a style="color:#00a0e9;" target="_blank" href='${loc}/space/studio/homePage?id=${this.$route.query.id}'>${loc}/space/studio/homePage?id=${this.$route.query.id}</a>`
                studioInviteMember({
