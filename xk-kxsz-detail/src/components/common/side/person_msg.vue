    ...
    <div id="person_msg">
        <div class="person-info text-center">
            ...
            <div class="btn-row text-center" v-if="!isAdmin">
                <button @click="handleFollow(queryUId, hasFollowed)" :title="hasFollowed ? '取消关注' : '关注'" class="focus btn">
                    {{ hasFollowed ? '已关注' : '关注' }}
                </button>
                <button @click="openDialog" class="msg btn pr" title="发私信">
                    <span class="btn-txt">私信</span>
                </button>
...
<script>
    ...
    import inviteDialog from 'Common/popupWindow/invite_dialog.vue'
    ...
    export default {
        ...
        methods: {
            ...
            async requestInterestList() { // 获取TA的兴趣组数据
                ...
                // const auditStatus = 1
                const res = await requestMyInterestList({ userId }, { pageSize: 5, pageNum: 1 })
            ...
            toInterestDetail(id) {
                ...
                // this.$router.push({path:'/space/interestDetail',query:{id:id}});
            },
            // 进入个人空间
