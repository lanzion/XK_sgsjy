                ...
                <el-table class="data-table" :data="dataList" style="width: 100%">
                    <el-table-column prop="prop0" :label="table_title[0]" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <template v-if="flag !== '/resource'">
                                <router-link v-if="inRoute" :to="{ path: linkTo.url || linkTo, query: { id: $route.query.id, [paramName]: scope.row.id } }" target="_blank">{{scope.row.prop0}}</router-link>
                                <a v-else @click.prevent="redirect(linkTo, { query: { id: $route.query.id, [paramName]: scope.row.id } })">{{scope.row.prop0}}</a>
                            </template>
...
export default {
    ...
    data() {
        return {
            ...
            inRoute: true,
            paramName: '',
            // 资源类型 (1.课题申报指南附件, 2.课题申报指南资源, 3.课题申报附件, 4.阶段成果, 5.课题成果, 6.课题任务)
            resourceType: {
                1: '课题申报指南附件',
                2: '课题申报指南资源',
                3: '课题申报附件',
            ...
            switch (this.flag) {
                ...
                case '/resource':
                    requestResourceList({ orgId: this.id, resourceType: this.typeId, name: this.search }, { pageSize: 5, pageNum: this.param.pageNum }).then((res) => {
                        const datas = res.data.entity ? res.data.entity.resultData : []
                        this.param.totalNum = res.data.entity.totalNum || 0
                        if (datas.length > 0) {
                            this.dataList = []
                            for (const i in datas) {
                                const obj = {}
