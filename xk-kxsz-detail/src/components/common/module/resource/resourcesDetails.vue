<template>
  <div id="resourcesDetails" class="theme-red backend-content__middle">
    ...
    <section class="preview-img">
    	<img :src="getFileUrl(detailsData.fileId)" height="684">
    </section>
    <div class="preview-msg">
    	<section class="collection-msg">
    		<div class="collection-title clearfix">
          <h5 class="fl">{{detailsData.name}}</h5>
          <com-panel-event module="resource" :title.sync="detailsData.name"></com-panel-event>
        </div>
    		<div class="collection-content">资源简介：{{detailsData.introduction}}</div>
    		<div class="collection-classify">分类：{{detailsData.resourceType}}</div>
    		<div class="collection-technology">技术：{{detailsData.technologyType}}</div>
	...
	.preview-msg{
        ...
        margin-bottom: 15px;
          ...
          padding-right: 50px;
			...
			.collection-content,.collection-classify,.collection-technology,.other-msg{
				font-size: 14px;
				line-height: 25px;
        width: 80%;
			}
