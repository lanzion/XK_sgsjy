            ...
            <ul class="evaluation-list" v-else>
                <li class="evaluation-item clearfix" v-for="(list, lIndex) in evaluationList" :key="lIndex">
                    ...
                    <div class="btn-group" v-if="list.createId != userId">
                        <v-report :report-data="{url: evaluateData.reportApi, params: {[evaluateData.reportKey]: list.id, createdId: userId }}"></v-report>
                    </div>
    ...
    watch: {
        ...
        evaluationDatas: {
            handler: function (val) {
                ...
                //     return item
                // })
                if (val.scoreCount !== undefined) {
                    const _scoreCount = val.scoreCount
                    this.$set(this.choiceStars[0], 'ct', val.totalEvalu)
                    return _scoreCount.filter((item, index) => {
                        if (item.ct === undefined) {
                            item.ct = 0
                        }
