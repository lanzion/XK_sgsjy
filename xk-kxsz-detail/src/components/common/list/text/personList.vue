<template>
    <div class="person-list-wrapper">
        <ul id="personList" class="tabs-person-list" v-if="datas.length > 0">
            <li v-for="(item, index) in datas" :key="index" class="tabs-person-item clearfix" @click.prevent="redirect(pathUrl, { query: { id: isGlory ? item.id : item.teacherId || item.studentId, uid: item.userId } })">
                <a class="head-face circle" :style='{backgroundImage: "url(" + (getFileUrl(item.userImage) || getFileUrl(item.cover) || faceDefault) + ")"}'></a>
                <article class="person-content">
                    <a class="person-title">
                    ...
                    <ul class="cols-list b-gray">
                        ...
                        <li class="cols-item" v-if="sideModel === 'student'">
                    ...
                    <div class="btn-group">
                        ...
                        <a @click.stop="showPrivateLetterDialog(item.userId)" class="btn-red i-button">
...
export default {
    ...
    methods: {
        // 关注、取消关注
        handleFollow(followedUId, hasFollowed, isStudent) {
