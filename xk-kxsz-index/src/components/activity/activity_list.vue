        ...
        <ul class="list" v-if="listData.length > 0">
            <li v-for="(item,index) in listData" class="clearfix">
                <div class="img-box fl">
                    <div class="bg-img__wrap" :style="{ backgroundImage: `url(${getFileUrl(item.cover) || listDefault})` }" @click="redirect(pages.detail.activity, { query: { id: item.id }})"></div>
                ...
                <div class="msg-box fl">
                    <a @click="redirect(pages.detail.activity, { query: { id: item.id }})">
...
.focus{
    border-color: nth($default-color, 1)!important;
}
#activity_list {
    width: 100%;
    overflow: hidden;
...
.status-box{
    ...
    .join-in{
        ...
        .dialog-box{
            border-top: 2px solid nth($default-color, 1)!important;
            .tips{
                width: 100%;
                margin-bottom: 5px;
                line-height: 15px;
