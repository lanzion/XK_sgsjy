<template>
    <!-- 省市区模块 -->
    <div id="region" class="category-box">
        <dl class="region-box category-box" v-for="(title, index) in regionTitle" v-if="regions[index].length" :class="{show}">
            <div v-if="index==0" class="more" @click="showMore" v-html='moreString'></div>
            <dd class="category-title"><span class="cate-tit">{{ title }}</span>:</dd>
