<template>
    <div id="list" :class="{'has-no-data': !items.length}">
        ...
        <!-- <ul class="list clearfix" v-if="items.length"> -->
            <li class="list-item__box" :class="{'list-item__shadow': ifShadow}" :style="{width: listWidth}" v-for="(item, index) in items"
                         :key="item.id || index" @click="redirect(pages.detail[listData.pathKey], { query: { id: item.id }})">
                <slot :item="item"></slot>
            </li>
        <!-- </ul> -->
        </transition-group>
