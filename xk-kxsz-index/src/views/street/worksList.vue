<template>
    <div class="container listBox">
        ...
        <div class="screeningBox bgc_w">
            <v-cascade class="category-box" model="works" :changeCascade="changeCascade"></v-cascade>
            ...
            <el-row :gutter="20" class="list-group__main" v-if="items.length">
                <el-col :span="6" v-for="(item, index) in items" :key="item.id || index">
                    <div class="list-group__item" @click="redirect(pages.detail.works, { query: { id: item.id }})">
                        ...
                        <div class="info">
                            <h6 class="item-title">{{ item.workName }}</h6>
...
export default {
    ...
    data() {
        return {
            ...
            tabs: [
                { name: '全部作品', type: 0 },
                { name: '最多浏览', type: 1, param: { browseNum: 1 } },
                { name: '最多点赞', type: 2, param: { praiseCount: 1 } },
                { name: '最多收藏', type: 3, param: { collectionCount: 1 } },
                { name: '最多分享', type: 4, param: { dynamicCount: 1 } },
            ],
            cur_tab: 0,
            is_search: true,
