        ...
        <section class="search-wrap">
            <div class="container search-box">
                <div class="search-input-box">
                    <input class="search-input" type="text" name="search" :value="$route.query.keyword" /><button class="search-button" type="button"><i class="el-icon-search"></i></button>
                </div>
                <div class="search-result-box pr">
                    <p class="txt pa" v-if="$route.query.keyword">共找到 <span class="total">{{total || 0}}</span> 条<span class="keyword">“{{$route.query.keyword}}”</span>相关</p>
                    <ul class="search-type-box">
                        <li class="search-type-item" v-for="(item, index) in nav" :key="index" :class="{active: activeNavIndex===index}">{{ item.name }}</li>
                    </ul>
                </div>
            </div>
        </section>
                        ...
                        <div class="list-group__item">
                            <div class="author-info table-layout">
                                <figure class="face-box table-cell">
                                    <a class="face bg-img__wrap" :style="{ 'background-image': `url(${getFileUrl(item.cover) || faceDefault})` }" @click="redirect(pages.detail[spaceNav[spaceNavActiveIndex].model], { query: { id: item.orgId, uid: item.userId } })"></a>
                                </figure>
                                <div class="base table-cell">
                                    <h6 class="item-title" @click="redirect(pages.detail[spaceNav[spaceNavActiveIndex].model], { query: { id: item.orgId, uid: item.userId } })">{{ item.teacherName || item.studentName }}（{{spaceNav[spaceNavActiveIndex].name}}）</h6>
